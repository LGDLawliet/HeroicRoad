...
Middle_Reapers_Scythe = Middle_Reapers_Scythe or class({})
LinkLuaModifier("modifier_Middle_Reapers_Scythe", "skills/Middle_Reapers_Scythe", LUA_MODIFIER_MOTION_NONE)

function Middle_Reapers_Scythe:OnSpellStart()
	local target = self:GetCursorTarget()
	self:Reap(target,1)
end

function Middle_Reapers_Scythe:Reap(target,index)
	...
	target:AddNewModifier(caster, self, "modifier_Middle_Reapers_Scythe", {duration = self:GetSpecialValueFor("stun_duration")})
	target:Poison(caster,self,addpoison)
end

----------------------------------------------------------------------------------------------------------------------
modifier_Middle_Reapers_Scythe = advanced_modifier({})
function modifier_Middle_Reapers_Scythe:IsDebuff() return true end 
function modifier_Middle_Reapers_Scythe:IsHidden() return true end 
function modifier_Middle_Reapers_Scythe:IsPurgable() return true end 
function modifier_Middle_Reapers_Scythe:IgnoreTenacity() return true end  --忽略韧性
function modifier_Middle_Reapers_Scythe:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
function modifier_Middle_Reapers_Scythe:GetEffectName()return "particles/units/heroes/hero_necrolyte/necrolyte_scythe.vpcf" end
function modifier_Middle_Reapers_Scythe:StatusEffectPriority()return MODIFIER_PRIORITY_ULTRA end
function modifier_Middle_Reapers_Scythe:GetPriority() return MODIFIER_PRIORITY_ULTRA end
function modifier_Middle_Reapers_Scythe:GetEffectAttachType() return PATTACH_ABSORIGIN_FOLLOW end

function modifier_Middle_Reapers_Scythe:OnCreated()
	if IsServer() then
		local caster = self:GetCaster()
...
function modifier_Middle_Reapers_Scythe:OnRefresh()
	if IsServer() then
		local caster = self:GetCaster()
		local target = self:GetParent()
		self.ability = self:GetAbility()
...
function modifier_Middle_Reapers_Scythe:CheckState()
	local state ={
...
function modifier_Middle_Reapers_Scythe:DeclareFunctions()
	local decFuncs =
		{
...
function modifier_Middle_Reapers_Scythe:GetOverrideAnimation()
	return ACT_DOTA_DISABLED
end

function modifier_Middle_Reapers_Scythe:OnDestroy()
	if IsServer() then
		local caster = self:GetCaster()
		local target = self:GetParent()
