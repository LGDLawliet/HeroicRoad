...
--------------------------------------------------------------------------------
modifier_generic_ring_lua = advanced_modifier({})
...
function modifier_generic_ring_lua:OnIntervalThink()
	...
	local targets = FindUnitsInRadius(
		...
		radius+self.width*0.5,	-- float, radius. or use FIND_UNITS_EVERYWHERE
    ...
    -- DebugDrawCircle( self:GetParent():GetOrigin(), Vector( 0, 255, 0 ), 100, radius+self.width*0.5, false, 0.03 )
    -- DebugDrawCircle( self:GetParent():GetOrigin(), Vector( 0, 0, 255 ), 255, math.max(radius-self.width*0.5,10), false, 0.03 )
	...
	for _,target in pairs(targets) do
		...
		if not self.targets[target] then
			...
			if (not self.IsCircle) or (target:GetOrigin()-self:GetParent():GetOrigin()):Length2D()>(radius-self.width*0.5) then
