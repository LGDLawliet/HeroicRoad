...
--
-- Modifier_wearable_hider_while_model_changes is a modifier which will hide all wearable when model changes,
-- e.g., hexed by Lion. It should be add to the unit when it is first spawned using 
--hUnit:AddNewModifier(hUnit, nil, "modifier_wearable_hider_while_model_changes", {})
--.sOriginalModel = "path/to/orginal/model.vmdl".
-- The sOriginalModel field of the modifier should be updated every time the original model of the unit is changed.
--
-- New wearable could be added using WearableManager:AddNewWearable(). 
--It accept two parameters: the handle of the unit and a table contains information of the wearable. 
-- If the library works in tools mode, it will load the items_game.txt file, which contains information of the wearables.
-- The second parameter could be as simple as {ID = "12345"}. If the wearable has multiple style, 
--you could specify it like {ID = "12345", style = "1"}. 
--Generally speaking this will be sufficient to load the wearable and it's particles. 
--However for new cosmetics, Valve somewhat hide their particle information.
-- You will have to add particle information by yourself.
-- The format of the second parameter will then become 
--{ID = "12345", style(optional) = "1",
-- particle_systems = {{system = "path/to/.vpcf",
-- attach_type = SOMETHING_LIKE_PATTACH_CUSTOMORIGIN_FOLLOW,
-- attach_entity(optional, default value is "self",
-- which means it will attach to the wearable. If this value is "parent", 
--it will attach to parent) = "parent" ,control_points(optional) = {{control_point_index = 0, 
--attach_type(optional, default value is PATTACH_POINT_FOLLOW) = SOMETHING_LIKE_PATTACH_POINT_FOLLOW, 
--attachment(optional) = "the_attachment_name_in_model_file"}, {(other control points)}}}, {(other systems)}}}.
-- Notice in this scenario, you have to dig how to add the particles. 
--The items_game.txt file itself contains particles the wearable uses (just cannot be load into lua), you can test them by yourself.
-- After adding new variable, it's information will be stored in the hUnit.tWearables table.
-- You can remove an added wearable by its index in items_game.
--txt with WearableManager:RemoveWearableByIndex(hUnit, sID) or remove all wearable with WearableManager:RemoveAllWearable(hUnit).
-- After successfully add a wearable,
-- you can output the information about the variable so you can use them in non-tools mode so you don't have to load the
-- big-ass items_game.txt file into your vscript system during playtime. Information of a single wearable could be
-- WearableManager:PrintWearableInfo(tWearableInfo). tWearableInfo is a element in the hUnit.tWearables table.
-- A second parameter could be added to specify the output file handle. A third parameter could be added to specify the nesting level (default is 0). 
--You can also output all wearable infos on a unit by using WearableManager:PrintAllWearableInfos(hUnit, hFile, iNestLevel). 
--The output info table could be directly used by WearableManager:AddNewWearable() without loading items_game.txt
--
-- Also, WearableManager:PrintAllPrecaches(hUnit) will print precaches you need for the wearables the unit has in addon_game_mode.lua file.
-- Notice that some cosmetics (like arcana) contains particles not with type of "particle_create", 
--and these particles may be called by a special animation of the model. In order not to get bunch of red crosses when these animation happenes,
-- you will have to precache these particles as well.
...
LinkLuaModifier("modifier_wearable_hider_while_model_changes", "internal/wearable_manager.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_wearable_visuals_illusion", "internal/wearable_manager.lua", LUA_MODIFIER_MOTION_NONE)
