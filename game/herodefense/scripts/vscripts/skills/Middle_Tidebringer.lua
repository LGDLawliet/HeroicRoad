...
Middle_Tidebringer = Middle_Tidebringer or class({})
LinkLuaModifier("modifier_Middle_Tidebringer", "skills/Middle_Tidebringer", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Middle_Tidebringer_debuff", "skills/Middle_Tidebringer", LUA_MODIFIER_MOTION_NONE)

function Middle_Tidebringer:Precache( context )
	PrecacheResource( "particle", "particles/units/heroes/hero_kunkka/kunkka_spell_tidebringer.vpcf", context )
end
function Middle_Tidebringer:GetIntrinsicModifierName()	return "modifier_Middle_Tidebringer" end
function Middle_Tidebringer:GetCastRange(location, target)
	return self:GetCaster():Script_GetAttackRange()
end
-- function Middle_Tidebringer:GetCooldown( nLevel )
...
modifier_Middle_Tidebringer =modifier_Middle_Tidebringer or class({})
function modifier_Middle_Tidebringer:IsHidden()	return true end
function modifier_Middle_Tidebringer:RemoveOnDeath()	return false end
function modifier_Middle_Tidebringer:IsPurgable()	return false end
function modifier_Middle_Tidebringer:DeclareFunctions()
	return {
...
function modifier_Middle_Tidebringer:OnCreated()
	-- local caster = self:GetCaster()
...
function modifier_Middle_Tidebringer:OnRefresh()
	-- local caster = self:GetCaster()
	-- local ability = self:GetAbility()
	if IsServer() then
...
function modifier_Middle_Tidebringer:OnAttackStart( params )
	if IsClient() then
		return
	end
...
function modifier_Middle_Tidebringer:OnAttackLanded( keys )
	if IsClient() then
		return
	end
	local ability = self:GetAbility()
	...
	if keys.attacker == parent and self.pass_attack then
		...
		if target ~= nil and target:GetTeamNumber() ~= self:GetParent():GetTeamNumber() then
			...
			local duration = 5
			keys.target:AddNewModifier(keys.attacker,ability, "modifier_Middle_Tidebringer_debuff", {duration = duration})
...
function modifier_Middle_Tidebringer:GetModifierPreAttack_BonusDamage(params)
	self.bonus_damage = self.bonus_damage or 0
	return self.bonus_damage
end


function modifier_Middle_Tidebringer:OnDamageCalculated(keys)
	if IsServer() then
...
modifier_Middle_Tidebringer_debuff = modifier_Middle_Tidebringer_debuff or advanced_modifier({})

function modifier_Middle_Tidebringer_debuff:IsDebuff() return true end
function modifier_Middle_Tidebringer_debuff:IsHidden() return false end
function modifier_Middle_Tidebringer_debuff:IsPurgable() return false end
-- function modifier_Middle_Tidebringer_debuff:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_Middle_Tidebringer_debuff:DeclareFunctions()
	return {
		MODIFIER_PROPERTY_TOOLTIP,             --护甲
	}
end
function modifier_Middle_Tidebringer_debuff:OnCreated(params)
...
function modifier_Middle_Tidebringer_debuff:OnRefresh(params)
	if IsServer() then
...
function modifier_Middle_Tidebringer_debuff:OnIntervalThink()
	if IsServer() then
...
function modifier_Middle_Tidebringer_debuff:OnTooltip()
    return self:Advanced_GetModifierPhysicalArmorBonus()
end

-- advanced_modifier

function modifier_Middle_Tidebringer_debuff:ADDeclareFunctions()
    return 
    {
        advanced_MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS,
    }
end
function modifier_Middle_Tidebringer_debuff:Advanced_GetModifierPhysicalArmorBonus()
...
-- modifier_Middle_Tidebringer_debuff