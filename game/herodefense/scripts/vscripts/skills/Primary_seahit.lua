LinkLuaModifier( "modifier_Primary_seahit_motion", "skills/Primary_seahit.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Primary_seahit_fly", "skills/Primary_seahit.lua", LUA_MODIFIER_MOTION_NONE )

Primary_seahit = class({})

function Primary_seahit:GetAOERadius()
	return self:GetSpecialValueFor("radius")
end

function Primary_seahit:GetCastRange(location , target)
	...
	for _, enemy in ipairs(enemies) do
		if enemy ~= nil and not enemy:IsMagicImmune() then
			...
			if enemy:IsAlive() then
				enemy:AddNewModifier(caster, self, "modifier_Primary_seahit_fly", {duration = 0.5})
			end
		end
	end
end

function Primary_seahit:OnSpellStart()
	local caster = self:GetCaster()
	-- 冲锋的部分
	...
	caster:AddNewModifier(caster, self, "modifier_Primary_seahit_motion", {duration = duration, pos_x = pos.x, pos_y = pos.y, pos_z = pos.z})
	caster:EmitSound("Hero_Morphling.Waveform")
	ProjectileManager:ProjectileDodge(caster)
end

---------------------------------------------------------------------
modifier_Primary_seahit_motion = advanced_modifier({})

function modifier_Primary_seahit_motion:IsDebuff()			return false end
function modifier_Primary_seahit_motion:IsHidden() 			return true end
function modifier_Primary_seahit_motion:IsPurgable() 		return false end
function modifier_Primary_seahit_motion:IsPurgeException() 	return false end
function modifier_Primary_seahit_motion:IsStunDebuff()		return true end
--状态
function modifier_Primary_seahit_motion:CheckState() 
	return 
...
function modifier_Primary_seahit_motion:DeclareFunctions() return {MODIFIER_PROPERTY_OVERRIDE_ANIMATION, MODIFIER_PROPERTY_DISABLE_TURNING} end
function modifier_Primary_seahit_motion:GetModifierDisableTurning() return 1 end
function modifier_Primary_seahit_motion:GetOverrideAnimation() return ACT_DOTA_CAST_ABILITY_1 end
function modifier_Primary_seahit_motion:IsMotionController() return true end
function modifier_Primary_seahit_motion:GetMotionControllerPriority() return DOTA_MOTION_CONTROLLER_PRIORITY_HIGH end

function modifier_Primary_seahit_motion:OnCreated(keys)
	local caster = self:GetCaster()
...
function modifier_Primary_seahit_motion:OnIntervalThink()
	local caster = self:GetCaster()
	local ability = self:GetAbility()
	local parent = self:GetParent()
	...
	-- 		unit:AddNewModifier(caster, ability, "modifier_Primary_seahit_Upgrade", {duration=self.wave_duration*gain})
	-- 		table.insert(self.hitted_friendly,unit)
	-- 	end
	-- end
end

function modifier_Primary_seahit_motion:OnDestroy() 
	local parent = self:GetParent()
...
function modifier_Primary_seahit_motion:ADDeclareFunctions()
	return {
		advanced_MODIFIER_PROPERTY_INCOMING_DAMAGE_PERCENTAGE,
	}
end
function modifier_Primary_seahit_motion:Advanced_GetModifierIncomingDamage_Percentage(params)
	if not self:GetAbility() then return end
	return -self.incoming
end
----------------------
modifier_Primary_seahit_fly = advanced_modifier({})

function modifier_Primary_seahit_fly:IsDebuff()				return true end
function modifier_Primary_seahit_fly:IsHidden() 			return true end
function modifier_Primary_seahit_fly:IsPurgable() 			return false end
function modifier_Primary_seahit_fly:DeclareFunctions() return {MODIFIER_PROPERTY_OVERRIDE_ANIMATION} end
function modifier_Primary_seahit_fly:GetOverrideAnimation() return ACT_DOTA_FLAIL end
function modifier_Primary_seahit_fly:CheckState() return {[MODIFIER_STATE_STUNNED] = true} end
function modifier_Primary_seahit_fly:OnRefresh(keys) self:OnCreated(keys) end
function modifier_Primary_seahit_fly:IsMotionController() return true end
function modifier_Primary_seahit_fly:GetMotionControllerPriority() return DOTA_MOTION_CONTROLLER_PRIORITY_HIGH end
function modifier_Primary_seahit_fly:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_Primary_seahit_fly:OnCreated(keys)
...
function modifier_Primary_seahit_fly:OnIntervalThink()
...
function modifier_Primary_seahit_fly:OnDestroy()
	if IsServer() then
