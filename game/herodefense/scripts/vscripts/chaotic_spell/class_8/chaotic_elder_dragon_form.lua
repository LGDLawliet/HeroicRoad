...
chaotic_elder_dragon_form = class({})
LinkLuaModifier("modifier_chaotic_elder_dragon_form_transform", "chaotic_spell/class_8/chaotic_elder_dragon_form", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_chaotic_elder_dragon_form_transform_cd", "chaotic_spell/class_8/chaotic_elder_dragon_form", LUA_MODIFIER_MOTION_NONE)
-- 指示器
function chaotic_elder_dragon_form:Precache( context )
...
function chaotic_elder_dragon_form:GetIntrinsicModifierName()
	return "modifier_generic_custom_indicator"
end

function chaotic_elder_dragon_form:CastFilterResultLocation( vLoc )
	if IsClient() then
...
function chaotic_elder_dragon_form:CreateCustomIndicator()
	local particle_cast = "particles/ui_mouseactions/custom_sector_finder.vpcf"
	self.effect_cast = ParticleManager:CreateParticle( particle_cast, PATTACH_ABSORIGIN_FOLLOW, self:GetCaster() )
end

function chaotic_elder_dragon_form:UpdateCustomIndicator( loc )
...
function chaotic_elder_dragon_form:DestroyCustomIndicator()
	ParticleManager:DestroyParticle( self.effect_cast, true ) 
	ParticleManager:ReleaseParticleIndex( self.effect_cast )

end

function chaotic_elder_dragon_form:GetCastRange()
	if IsServer() then
		return 30000
	end
	local caster = self:GetCaster()
...
-- 施法效果
function chaotic_elder_dragon_form:OnSpellStart()
	local caster = self:GetCaster()
	local ability = self
	...
	caster:GameTimer(0.2,function ()
		...
		caster.Form_MODIFIER_NAME = "modifier_chaotic_elder_dragon_form_transform"
		-- 变龙
		...
		caster:AddNewModifier(caster, ability, "modifier_chaotic_elder_dragon_form_transform", {duration = duration})
	end)
end

function chaotic_elder_dragon_form:Fireshoot(pos, damage_index, radius_index)
	if not IsServer() then return end
	local caster = self:GetCaster()
	-- 传参，伤害，长度，宽度，速度
	...
	if self.type == 1 then
		length = length + caster:Script_GetAttackRange()*0.5
	end
...
function chaotic_elder_dragon_form:OnProjectileHit_ExtraData( target, location,keys )
...
function chaotic_elder_dragon_form:PlayEffect(target)
...
-------------------------------------------------------------------------------------------------------------------------------
modifier_chaotic_elder_dragon_form_transform = advanced_modifier({})
function modifier_chaotic_elder_dragon_form_transform:IsHidden()	return false end
function modifier_chaotic_elder_dragon_form_transform:IsPurgable()	return false end
function modifier_chaotic_elder_dragon_form_transform:IsDebuff()	return false end
function modifier_chaotic_elder_dragon_form_transform:GetPriority() return MODIFIER_PRIORITY_ULTRA-1 end
function modifier_chaotic_elder_dragon_form_transform:DeclareFunctions()	
	local decFuncs = {
...
-- 更改形态，更换弹道和攻击声音
function modifier_chaotic_elder_dragon_form_transform:GetModifierModelScale() 
    return 40
end
function modifier_chaotic_elder_dragon_form_transform:GetAttackSound()
	return "Hero_DragonKnight.ElderDragonShoot2.Attack"
end
function modifier_chaotic_elder_dragon_form_transform:GetModifierModelChange()
	return "models/items/dragon_knight/oblivion_blazer_dragon/oblivion_blazer_dragon.vmdl"
end
function modifier_chaotic_elder_dragon_form_transform:GetModifierProjectileName()
...
-- 改变攻击形态
function modifier_chaotic_elder_dragon_form_transform:OnCreated()
...
-- 退出变身，属性复原
function modifier_chaotic_elder_dragon_form_transform:OnDestroy()
    if IsServer() then    	
		local caster =self:GetCaster()
		--如果单位不是远程单位 则改变为远程
...
-- 攻击力，攻击距离，弹道速度
function modifier_chaotic_elder_dragon_form_transform:ADDeclareFunctions()
    return 
    {
...
function modifier_chaotic_elder_dragon_form_transform:Advanced_GetModifierPreAttack_BonusDamage() return self.bonus_attack end
function modifier_chaotic_elder_dragon_form_transform:Advanced_GetModifierAttackRangeBonus() return  self.bonus_attack_range end
function modifier_chaotic_elder_dragon_form_transform:GetModifierProjectileSpeedBonus() return 200 end

-- 火焰A
function modifier_chaotic_elder_dragon_form_transform:OnAttack(keys)
	if not IsServer() or keys.attacker ~= self:GetParent() then
		return
	end
	if keys.attacker:IsInSpecialAttack() then return end
	if keys.attacker:HasModifier("modifier_chaotic_elder_dragon_form_transform_cd") then return end
	...
	if self.type == 1 then
		...
		local chance = self.ability:GetSpecialValueFor("rune_1_chance")
		if chance >= random(1,100) then
			return 
		end
	end

	keys.attacker:AddNewModifier(keys.attacker,self:GetAbility(),"modifier_chaotic_elder_dragon_form_transform_cd",{duration = cd})
end
-- 溅射攻击
function modifier_chaotic_elder_dragon_form_transform:OnAttackLanded(keys)
	if not IsServer() then return end
	...
	if self.type == 2 then
		cleave_damage = cleave_damage * (1+self.ability:GetSpecialValueFor("rune_2_cleave")*0.01)
	end
...
-------------------------------------------------------------------------------------------------------------------------------
modifier_chaotic_elder_dragon_form_transform_cd = advanced_modifier({})
function modifier_chaotic_elder_dragon_form_transform_cd:IsHidden()	return true end
function modifier_chaotic_elder_dragon_form_transform_cd:IsPurgable()	return false end
function modifier_chaotic_elder_dragon_form_transform_cd:IsDebuff()	return false end