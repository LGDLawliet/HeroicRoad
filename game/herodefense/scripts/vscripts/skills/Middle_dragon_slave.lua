Middle_dragon_slave = class({})


function Middle_dragon_slave:GetCastRange()
	-- local caster =self:GetCaster()
	return self:GetSpecialValueFor("range")

end

function Middle_dragon_slave:OnSpellStart()
	-- unit identifier
	local caster = self:GetCaster()
...
--------------------------------------------------------------------------------
-- Projectile
function Middle_dragon_slave:OnProjectileHitHandle( target, location, projectile )
	...
	local damageTable = {
		...
		damage = (self:GetSpecialValueFor("damage")+self:GetSpecialValueFor("bonus_damage")*self:GetCaster():GetIntellect(false))*self.ProjectileTable[projectile],
		damage_type = self:GetAbilityDamageType(),
		ability = self, --Optional.
		hd_flags = HD_DAMAGE_FLAG_FIRE_DAMAGE,
	}
	ApplyDamage( damageTable )
	self.ProjectileTable[projectile] = self.ProjectileTable[projectile] +0.04
	-- get direction
...
--------------------------------------------------------------------------------
function Middle_dragon_slave:PlayEffects( target, direction )
	-- Get Resources
