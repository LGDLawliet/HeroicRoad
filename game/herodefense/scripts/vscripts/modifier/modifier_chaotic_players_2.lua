...
modifier_chaotic_players_2 = advanced_modifier({})

function modifier_chaotic_players_2:IsHidden()return false end
function modifier_chaotic_players_2:IsDebuff()return false end
function modifier_chaotic_players_2:IsPurgable()return false end
function modifier_chaotic_players_2:GetTexture() return "roshan_bash" end
function modifier_chaotic_players_2:IsPurgeException() 	return false end
function modifier_chaotic_players_2:RemoveOnDeath() return false end
function modifier_chaotic_players_2:OnCreated(keys)
	if IsServer() then
		self:SetStackCount(2)
		self:StartIntervalThink(15)
	end
end
function modifier_chaotic_players_2:ADDeclareFunctions()
    return 
    {
...
function modifier_chaotic_players_2:Advanced_GetModifierTotalDamageOutgoing_Percentage_Mul()
	return 90
end

function modifier_chaotic_players_2:OnTakeDamage(tg)
    if IsServer() then   
		...
		local hp = tg.damage *0.03 *life_steal_gain
...
function modifier_chaotic_players_2:OnSummonUnit(keys)
	if IsServer() then
		local unit = keys.target
        unit:AddNewModifier(self:GetParent(), nil, "modifier_chaotic_players_2", {})
	end
end
function modifier_chaotic_players_2:OnIntervalThink()
	local parent = self:GetParent()
...
function modifier_chaotic_players_2:AdvancedGetModifierReincarnate(keys)
...
function modifier_chaotic_players_2:OnReincarnateTrigger(keys)
