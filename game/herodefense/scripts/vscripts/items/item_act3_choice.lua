LinkLuaModifier("modifier_item_act3_drop", "items/item_act3_choice.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_hd_rubick_cube2_active", "items/item_hd_rubick_cube", LUA_MODIFIER_MOTION_NONE)
item_act3_choice = class({})

function item_act3_choice:OnSpellStart()
    local caster = self:GetCaster()
    -- caster:AddNewModifier(caster, self, "modifier_item_hd_rubick_cube2_active", {})

	local items = {}
	local itemData = {
		itemName = "item_act3_wind",
		itemType = "class",
	}
    local itemData2 = {
		itemName = "item_act3_earth",
		itemType = "class",
	}
    local itemData3 = {
		itemName = "item_act3_water",
		itemType = "class",
	}
    local itemData4 = {
		itemName = "item_act3_fire",
		itemType = "class",
	}
    local itemData5 = {
		itemName = "item_act3_chaos_unupdate",
		itemType = "class",
	}
	-- local itemData6 = {
	-- 	itemName = "item_act3_wind",
	...
	local caster = self:GetCaster()
    local item = caster:FindItemInInventory("item_act3_choice")
    if item ~=nil then
...
----
modifier_item_act3_drop = advanced_modifier({})

function modifier_item_act3_drop:IsHidden() return true end
function modifier_item_act3_drop:IsDebuff() return false end
function modifier_item_act3_drop:IsPurgable() return false end
function modifier_item_act3_drop:RemoveOnDeath() return false end
function modifier_item_act3_drop:OnCreated()
	if IsServer() then
...
function modifier_item_act3_drop:ADDeclareFunctions()
    return {
...
function modifier_item_act3_drop:OnDeath(params)
    if not IsServer() then return end
	local unit = params.unit
	local attacker = params.attacker
	if attacker:GetPlayerOwnerID() ~= self:GetParent():GetPlayerOwnerID() then return end
...
function modifier_item_act3_drop:OnAbilityFullyCast(params)
    if not IsServer() then return end
    ...
    if params.ability:GetName() == "item_secret_of_experience" 
		...
		if self.book_chance >= math.random(1,100) then
			self:GetParent():AddItemByName("item_act3_giant")
		end
	end
	--每使用10次风险骰子
	--谁用的给谁
...
function modifier_item_act3_drop:Gainitems()
	if not IsServer() then return end
	local heroes = GetAllRealHeroes()
	for _, hero in pairs(heroes) do
		if hero ~= self.parent then
			if 60 >= math.random(1,100) then
				hero:AddItemByName("item_act3_giant")
			end
		else
			hero:AddItemByName("item_act3_giant")
		end
