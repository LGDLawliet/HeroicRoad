modifier_diff_4 = advanced_modifier({})

function modifier_diff_4:IsHidden()return false end
function modifier_diff_4:IsDebuff()return false end
function modifier_diff_4:IsPurgable()return false end
function modifier_diff_4:IsPurgeException() 	return false end
function modifier_diff_4:RemoveOnDeath() return true end
function modifier_diff_4:GetAttributes() return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
function modifier_diff_4:GetTexture() return self.texture end

function modifier_diff_4:OnCreated(keys)
    if IsServer() then
...
function modifier_diff_4:ModifyTaskData(data)
...
--每过回合征召怪物cy
function modifier_diff_4:ADDeclareFunctions()
    return 
    {
...
function modifier_diff_4:OnChaoticEraRoundChange(keys)
    local round = keys.round
    local round_index = 0.2 + round/34
    if round >= 24 then
        round_index = round_index + round/68
        if round >= 30 then
...
function modifier_diff_4:AdvancedModifyChaoticEraSpwnData(attribute,unit)
    if unit then
        ...
        if self.skilltable and type(self.skilltable) == "table" then
            ...
            -- 回合数不同生成的词条数目也应该不同,0-12回合是0-1个，13-24回合是1-2个，25-34回合是2个
            print("wave="..wave)
            local count = 0
            if wave >= 13 then
                count = 1
