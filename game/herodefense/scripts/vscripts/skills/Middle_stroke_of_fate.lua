Middle_stroke_of_fate = class({})
LinkLuaModifier( "modifier_Middle_stroke_of_fate", "skills/Middle_stroke_of_fate", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_stroke_of_fate_buff", "skills/Middle_stroke_of_fate", LUA_MODIFIER_MOTION_NONE )
--------------------------------------------------------------------------------
-- Ability Phase Start
function Middle_stroke_of_fate:OnAbilityPhaseStart()
	-- play effects
...
function Middle_stroke_of_fate:OnAbilityPhaseInterrupted()
	self:StopEffects1()
...
function Middle_stroke_of_fate:OnSpellStart()
	-- unit identifier
...
--------------------------------------------------------------------------------
-- Projectile
Middle_stroke_of_fate.active_proj = {}
function Middle_stroke_of_fate:OnProjectileHitHandle( target, location, handle )
	if IsServer() then
		if not target then
		...
		if self.active_proj[handle].bonus_count<5 then
			...
			if modifier then
				...
				self:EndCooldown()
				if cooldown-stack*0.1>0 then
					self:StartCooldown(cooldown-stack*0.1)
				end
				print("触发1")
		...
		-- debuff
		target:AddNewModifier(self:GetCaster(), self,"modifier_Middle_stroke_of_fate",{	duration = slow*StatusResistance,} )
...
--------------------------------------------------------------------------------
function Middle_stroke_of_fate:PlayEffects1(target)
	-- Get Resources
...
function Middle_stroke_of_fate:StopEffects1()
	-- stop effects
	local sound_precast = "Hero_Grimstroke.DarkArtistry.PreCastPoint"
	StopSoundOn( sound_precast, self:GetCaster() )
end

function Middle_stroke_of_fate:PlayEffects2( target )
	-- Get Resources
...
modifier_Middle_stroke_of_fate = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_Middle_stroke_of_fate:IsHidden()	return false end
function modifier_Middle_stroke_of_fate:IsDebuff()	return true end
function modifier_Middle_stroke_of_fate:IsStunDebuff()	return false end
function modifier_Middle_stroke_of_fate:IsPurgable()	return true end

--------------------------------------------------------------------------------
-- Initializations
function modifier_Middle_stroke_of_fate:OnCreated( kv )
	-- references
	self.slow = self:GetAbility():GetSpecialValueFor( "move_slow" ) -- special value
end

function modifier_Middle_stroke_of_fate:OnRefresh( kv )
	-- references
	self.slow = self:GetAbility():GetSpecialValueFor( "move_slow" ) -- special value	
end
...
--------------------------------------------------------------------------------
-- Modifier Effects
function modifier_Middle_stroke_of_fate:DeclareFunctions()
	local funcs = {
...
function modifier_Middle_stroke_of_fate:GetModifierMoveSpeedBonus_Constant()	return -self.slow end

--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_Middle_stroke_of_fate:GetEffectName()
	return "particles/units/heroes/hero_grimstroke/grimstroke_dark_artistry_debuff.vpcf"
end

function modifier_Middle_stroke_of_fate:GetEffectAttachType()	return PATTACH_ABSORIGIN_FOLLOW end
