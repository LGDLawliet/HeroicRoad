LinkLuaModifier( "modifier_Advanced_Anti_time", "skills/Advanced_Anti_time", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_time_already", "skills/Advanced_Anti_time", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Advanced_Soul", "skills/Advanced_Anti_time", LUA_MODIFIER_MOTION_NONE )
Advanced_Anti_time = class({})
function Advanced_Anti_time:IsRefreshable() return false end
function Advanced_Anti_time:Precache( context )
...
function Advanced_Anti_time:CheckKV(key)
	local table = {
		attack_speed = 0.5,
		cd_speed = 3,
		duration = 0.2
...
function Advanced_Anti_time:UnlockFirstCore(key)
	return true
end
function Advanced_Anti_time:UnlockSecondCore(key)
	return true
end
function Advanced_Anti_time:UnlockThirdCore(key)
	return true
end
function Advanced_Anti_time:GetCustomCastError()
	return "#DOTA_CUSTOM_ALL_TIRED"
end

function Advanced_Anti_time:CastFilterResult()
	if IsServer() then
...
function Advanced_Anti_time:OnSpellStart()
	local caster = self:GetCaster()
	local heroes = GetAllRealHeroes()
	...
	if self:GetSpecialValueFor("advanced_level") >= 15 then
		tired_duration = tired_duration - 30
	end
	for _,hero in pairs(heroes) do
		if not hero:HasModifier("modifier_time_already") then
			...
			-- end
			hero:AddNewModifier(caster,self,"modifier_Advanced_Anti_time",{duration = duration})
		end
	end
	local soul_num = self:GetSpecialValueFor("count")
	if self:GetSpecialValueFor("advanced_level") >= 20 then
		soul_num = soul_num + 1
	end
	self:CreateSouls(soul_num)
end

function Advanced_Anti_time:CreateSouls(num)
	local caster = self:GetCaster()
	...
	for i = 1, num do
		...
		newSoul:AddNewModifier(caster,self,"modifier_Advanced_Soul",{duration = duration})

	end
end
----
modifier_Advanced_Soul = modifier_Advanced_Soul or advanced_modifier({})
function modifier_Advanced_Soul:OnCreated()
	self:StartIntervalThink(3)
end
function modifier_Advanced_Soul:OnIntervalThink()
	if not self:GetAbility() then self:Destroy() return end
end
function modifier_Advanced_Soul:CheckState()
	local state = {
...
function modifier_Advanced_Soul:OnDestroy()
	if IsServer() then
		UTIL_Remove(self:GetParent())
	end
end
---------------------------------------------------------------------
modifier_Advanced_Anti_time = advanced_modifier({})

function modifier_Advanced_Anti_time:IsHidden()return false end
function modifier_Advanced_Anti_time:IsDebuff()return false end
function modifier_Advanced_Anti_time:RemoveOnDeath()return false end
function modifier_Advanced_Anti_time:OnCreated(params)
...
function modifier_Advanced_Anti_time:OnRefresh(params)
	self.ability = self:GetAbility()
	self.caster = self:GetCaster()
	self.parent = self:GetParent()
...
function modifier_Advanced_Anti_time:OnDestroy()
...
function modifier_Advanced_Anti_time:OnIntervalThink()
	if not self:GetAbility() then self:Destroy() return end
	local keys = {
		target = self.parent,
...
function modifier_Advanced_Anti_time:TimeSoul()
...
function modifier_Advanced_Anti_time:PlayEffects(unit)
...
function modifier_Advanced_Anti_time:PlayEffects_TakeSoul(target)
...
function modifier_Advanced_Anti_time:ADDeclareFunctions()
	local funcs = {
...
function modifier_Advanced_Anti_time:DeclareFunctions()
	local decfuncs = {}
	if self:GetAbility():GetSpecialValueFor("advanced_level") >= 10 then
		table.insert(decfuncs,MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE)
		table.insert(decfuncs,MODIFIER_PROPERTY_IGNORE_MOVESPEED_LIMIT)
	end
	return decfuncs
end

function modifier_Advanced_Anti_time:Advanced_GetModifierAttackSpeedPercentage()
	if not self:GetAbility() then self:Destroy() return end
	return self.attack_speed
end
function modifier_Advanced_Anti_time:Advanced_GetModifier_CastPoint()
	if not self:GetAbility() then self:Destroy() return end
	return 1000
end
function modifier_Advanced_Anti_time:GetModifierMoveSpeedBonus_Percentage()
	if not self:GetAbility() then self:Destroy() return end
	return 50
end
function modifier_Advanced_Anti_time:GetModifierIgnoreMovespeedLimit()
	if not self:GetAbility() then self:Destroy() return end
	return 1
end
function modifier_Advanced_Anti_time:Advanced_GetAdvancedLevelBonus()
	if not self:GetAbility() then self:Destroy() return end
	return self.skill_level
end
function modifier_Advanced_Anti_time:Advanced_GetModifierBonusStats_Strength()
	if not self:GetAbility() then self:Destroy() return end
	return self.atb + self:GetStackCount()*15
end
function modifier_Advanced_Anti_time:Advanced_GetModifierBonusStats_Agility()
	if not self:GetAbility() then self:Destroy() return end
	return self.atb + self:GetStackCount()*15
end
function modifier_Advanced_Anti_time:Advanced_GetModifierBonusStats_Intellect()
	if not self:GetAbility() then self:Destroy() return end
	return self.atb + self:GetStackCount()*15
end
