...
Primary_earth_spike = class({})

function Primary_earth_spike:IsHiddenWhenStolen()
	return false
end 
function Primary_earth_spike:GetCastRange(vLocation, hTarget)
...
function Primary_earth_spike:OnSpellStart()
	-- 预防目标位置等于自身位置导致的bug
...
function Primary_earth_spike:OnProjectileHit_ExtraData(target, location, extra_data)
	...
	-- ParticleManager:SetParticleControl(particle_hit_fx, 0, GetGroundPosition(target_position, nil))
	-- ParticleManager:SetParticleControl(particle_hit_fx, 1, GetGroundPosition(target_position, nil))
	-- ParticleManager:SetParticleControl(particle_hit_fx, 2, GetGroundPosition(target_position, nil))
