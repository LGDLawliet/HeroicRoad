...
--------------------------------------------------------------------------------
Primary_Macropyre = class({})
LinkLuaModifier( "modifier_Primary_Macropyre", "skills/Primary_Macropyre", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Primary_Macropyre_thinker", "skills/Primary_Macropyre", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Primary_Macropyre_count", "skills/Primary_Macropyre", LUA_MODIFIER_MOTION_NONE )


--------------------------------------------------------------------------------
-- Custom KV
-- Cast Range
function Primary_Macropyre:GetCastRange( vLocation, hTarget )
	return self:GetSpecialValueFor( "cast_range" ) + self:GetCaster():GetCastRangeBonus()
end

function Primary_Macropyre:MakeMacropyreAt(start,point, duration)
	...
	local thinker = CreateModifierThinker(
		...
		self, -- ability source
		"modifier_Primary_Macropyre_thinker", -- modifier name
		{
			duration = duration,
			-- x = dir.x,
			-- y = dir.y,
	...
	table.insert(self.all_pyres, thinker:FindModifierByName("modifier_Primary_Macropyre_thinker"))
end
...
--------------------------------------------------------------------------------
function Primary_Macropyre:Spawn()
	if not self.all_pyres then self.all_pyres = {} end
end

-- Ability Start
function Primary_Macropyre:OnSpellStart()
	...
	local range = math.max(self:GetCastRange( point, nil ) + caster:GetCastRangeBonus(),100)
...
--------------------------------------------------------------------------
--thinker
modifier_Primary_Macropyre_thinker = class({})
require("internal/timers")
--------------------------------------------------------------------------------
-- Classifications
function modifier_Primary_Macropyre_thinker:IsHidden()     return false  end
function modifier_Primary_Macropyre_thinker:IsDebuff()     return false  end
function modifier_Primary_Macropyre_thinker:IsStunDebuff() return false end
function modifier_Primary_Macropyre_thinker:IsPurgable()   return false end
--------------------------------------------------------------------------------
-- Initializations


function modifier_Primary_Macropyre_thinker:OnCreated( kv )	
	if not IsServer() then return end
...
function modifier_Primary_Macropyre_thinker:OnRefresh( kv ) end
function modifier_Primary_Macropyre_thinker:OnRemoved()     end
function modifier_Primary_Macropyre_thinker:OnDestroy()
	if not IsServer() then return end
...
--------------------------------------------------------------------------------
-- Interval Effects
function modifier_Primary_Macropyre_thinker:OnIntervalThink()
	...
	local count_buffs = self.caster:FindAllModifiersByName("modifier_Primary_Macropyre_count")
	for _,enemy in pairs(enemies) do
		...
		enemy:AddNewModifier(
			...
			"modifier_Primary_Macropyre", -- modifier name
			{
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_Primary_Macropyre_thinker:PlayEffects()
	-- Get Resources
...
------------------------------------------------------------------------
--------------------------------------------------------------------------------
modifier_Primary_Macropyre = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_Primary_Macropyre:IsHidden()return false end
function modifier_Primary_Macropyre:IsDebuff()
...
function modifier_Primary_Macropyre:IsStunDebuff()return false  end
function modifier_Primary_Macropyre:IsPurgable()return false end

function modifier_Primary_Macropyre:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
-- --传火
-- function modifier_Primary_Macropyre:OnDeath( kv )
-- 	if kv.unit ~= self:GetParent()  or kv.unit:GetHealth() ~= 0 then return end
-- 	-- local macropyre = self:GetCaster():FindAbilityByName("Primary_Macropyre")
...
-- 		self:GetAbility(), -- ability source
-- 		"modifier_Primary_Macropyre_count", -- modifier name
-- 		{
...
--------------------------------------------------------------------------------
-- Initializations
function modifier_Primary_Macropyre:OnCreated( kv )
	local ability = self:GetAbility()
	if not ability or ability:IsNull() then
		self:SafeDestroy()
		return
...
function modifier_Primary_Macropyre:OnRefresh( kv )
	local ability = self:GetAbility()
	if not ability or ability:IsNull() then
		self:SafeDestroy()
		return
...
function modifier_Primary_Macropyre:OnRemoved() end
function modifier_Primary_Macropyre:OnDestroy() end

--------------------------------------------------------------------------------
-- Interval Effects
function modifier_Primary_Macropyre:OnIntervalThink()
	local ability = self:GetAbility()
	if not ability or ability:IsNull() then
		self:SafeDestroy()
		return
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_Primary_Macropyre:GetEffectName()
	return "particles/units/heroes/hero_jakiro/jakiro_liquid_fire_debuff.vpcf"
end

function modifier_Primary_Macropyre:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

--------------------------------------
modifier_Primary_Macropyre_count = class({})

function modifier_Primary_Macropyre_count:IsDebuff()				return false end
function modifier_Primary_Macropyre_count:IsHidden() 			    return true end
function modifier_Primary_Macropyre_count:IsPurgable() 			return false end
function modifier_Primary_Macropyre_count:IsPurgeException() 	    return false end
function modifier_Primary_Macropyre_count:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE + MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
