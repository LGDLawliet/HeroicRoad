Middle_Culling_Blade = class({})

LinkLuaModifier("modifier_Middle_Culling_Blade_sprint", "skills/Middle_Culling_Blade", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Middle_Culling_Blade_debuff", "skills/Middle_Culling_Blade", LUA_MODIFIER_MOTION_NONE)
function Middle_Culling_Blade:IsHiddenWhenStolen() 		return false end
function Middle_Culling_Blade:IsRefreshable() 			return true end
function Middle_Culling_Blade:IsStealable() 				return true end
function Middle_Culling_Blade:IsNetherWardStealable() 	return true end

function Middle_Culling_Blade:OnAbilityPhaseStart()
	self:GetCaster():StartGesture(ACT_DOTA_CAST_ABILITY_4)
	return true
end

function Middle_Culling_Blade:OnAbilityPhaseInterrupted() self:GetCaster():RemoveGesture(ACT_DOTA_CAST_ABILITY_4) end

function Middle_Culling_Blade:OnSpellStart()
	...
	if target:GetHealth() <= kill_threshold then
		...
		for _, ally in pairs(allies) do
			...
			ally:AddNewModifier(caster, self, "modifier_Middle_Culling_Blade_sprint", {duration = buff_duration})
		end
		...
		target:AddNewModifier(caster, self, "modifier_Middle_Culling_Blade_debuff", {duration = debuff_duration})
	end
end

modifier_Middle_Culling_Blade_sprint = class({})

function modifier_Middle_Culling_Blade_sprint:IsDebuff()				return false end
function modifier_Middle_Culling_Blade_sprint:IsPurgable() 			return true end
function modifier_Middle_Culling_Blade_sprint:IsPurgeException() 		return true end
function modifier_Middle_Culling_Blade_sprint:IsHidden()				return false end

function modifier_Middle_Culling_Blade_sprint:GetEffectName()	return "particles/units/heroes/hero_axe/axe_cullingblade_sprint.vpcf" end

function modifier_Middle_Culling_Blade_sprint:DeclareFunctions()
	return {MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,MODIFIER_PROPERTY_ATTACKSPEED_BONUS_CONSTANT}
end
function modifier_Middle_Culling_Blade_sprint:OnCreated()
	self.as_bonus = self:GetAbility():GetSpecialValueFor("as_bonus")
	self.move = self:GetAbility():GetSpecialValueFor("speed_bonus")
end
function modifier_Middle_Culling_Blade_sprint:OnRefresh()
	self.as_bonus = self:GetAbility():GetSpecialValueFor("as_bonus")
	self.move = self:GetAbility():GetSpecialValueFor("speed_bonus")
end
function modifier_Middle_Culling_Blade_sprint:GetModifierAttackSpeedBonus_Constant()	
...
function modifier_Middle_Culling_Blade_sprint:GetModifierMoveSpeedBonus_Percentage()	
	if not self:GetAbility() then
		self:Destroy()
		return 
	end
...
--额外受到伤害
modifier_Middle_Culling_Blade_debuff = advanced_modifier({})

function modifier_Middle_Culling_Blade_debuff:IsDebuff()			return true end
function modifier_Middle_Culling_Blade_debuff:IsHidden() 			return false end
function modifier_Middle_Culling_Blade_debuff:IsPurgable() 		return false end
function modifier_Middle_Culling_Blade_debuff:IsPurgeException() 	return true end
function modifier_Middle_Culling_Blade_debuff:OnCreated()
	self.incoming = self:GetAbility():GetSpecialValueFor("incoming")
end
function modifier_Middle_Culling_Blade_debuff:Advanced_GetModifierIncomingDamage_Percentage(keys) 
...
function modifier_Middle_Culling_Blade_debuff:ADDeclareFunctions()
