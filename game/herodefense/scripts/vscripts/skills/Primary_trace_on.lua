...
Primary_trace_on = class({})

LinkLuaModifier("modifier_Primary_trace_on", "skills/Primary_trace_on", LUA_MODIFIER_MOTION_NONE)

require('internal/timers')   --计时器功能
function Primary_trace_on:GetIntrinsicModifierName()
	return "modifier_Primary_trace_on"
end


modifier_Primary_trace_on = advanced_modifier({})

function modifier_Primary_trace_on:IsDebuff() return false end
function modifier_Primary_trace_on:IsHidden() return true end
function modifier_Primary_trace_on:IsPurgable() return false end


function modifier_Primary_trace_on:OnCreated(keys)
...
function modifier_Primary_trace_on:OnIntervalThink()
	if IsServer() then
