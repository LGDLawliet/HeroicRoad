...
Advanced_jim_magic_missile = class({})
function Advanced_jim_magic_missile:Precache( context )
...
function Advanced_jim_magic_missile:CheckKV(key)
	local table = {
		damage=20,
		bonus_damage =0.1,
	}
...
function Advanced_jim_magic_missile:GetAOERadius()
	return self:GetSpecialValueFor("radius")
end	
-- function Advanced_jim_magic_missile:GetM,anaCost(iLevel)
...
function Advanced_jim_magic_missile:GetCooldown(iLevel)
	if self:GetAutoCastState() and self:GetSpecialValueFor("advanced_level")>=15 then 
		return self:GetSpecialValueFor("cooldown_time")*0.5
	end
...
function Advanced_jim_magic_missile:OnSpellStart()
	...
	if self.modifier_origin then
		...
		if self:GetSpecialValueFor("advanced_level")>=25 and self:GetAutoCastState() and self.lvl25 ~= 1 then
			self.modifier_origin:SetStackCount(self.modifier_origin:GetStackCount() - 15000)
			self.lvl25 = 1
		end
	end
	...
	self:GetCaster():GameTimer(0.06, function()
		if IsValid(self) then
			...
			if IsValid(target) and target:IsAlive() then
				if IsServer() then
					if (not caster:HasModifier("modifier_jim_magic_missile_buff")) then
						self.modifier_origin = caster:AddNewModifier(caster, self, "modifier_jim_magic_missile_buff", {duration = -1})
			...
			if IsValid(target) and target:IsAlive() then
				...
				info.Target = target
				ProjectileManager:CreateTrackingProjectile(info)
				-- caster:EmitSound("Advanced_jim_magic_missile_cast")  
...
function Advanced_jim_magic_missile:OnProjectileHit_ExtraData(target, location, keys)
	local caster = self:GetCaster()
	if not target then
...
function Advanced_jim_magic_missile:ApplyModifier(target)
	local caster = self:GetCaster()
