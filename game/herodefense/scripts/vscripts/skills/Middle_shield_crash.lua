...
Middle_shield_crash = Middle_shield_crash or class({})
LinkLuaModifier( "modifier_Middle_shield_crash", "skills/Middle_shield_crash", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Middle_shield_crash_debuff", "skills/Middle_shield_crash", LUA_MODIFIER_MOTION_NONE )

function Middle_shield_crash:Precache( context )
...
function Middle_shield_crash:OnSpellStart()
	-- unit identifier
	...
	arc:SetEndCallback(function()
		...
		for i,enemy in pairs(enemies) do
			if i<=8 then
				...
				if enemy:IsAlive() then
					...
					enemy:AddNewModifier(caster, self, "modifier_Middle_shield_crash_debuff", {duration =debuff_duration*StatusResistance })
				end
		...
		if stack>0 then
			caster:AddNewModifier(
				caster, -- player source
				self, -- ability source
				"modifier_Middle_shield_crash", -- modifier name
				{
...
function Middle_shield_crash:PlayEffects1( modifier )
...
function Middle_shield_crash:PlayEffects2()
...
function Middle_shield_crash:PlayEffects3()
...
function Middle_shield_crash:PlayEffects4( target )
...
modifier_Middle_shield_crash =modifier_Middle_shield_crash or advanced_modifier({})
function modifier_Middle_shield_crash:IsHidden()	return false end
function modifier_Middle_shield_crash:IsDebuff()	return false end
function modifier_Middle_shield_crash:IsPurgable()	return true end
function modifier_Middle_shield_crash:OnCreated( kv )
...
function modifier_Middle_shield_crash:OnRefresh( kv )
	if not IsServer() then return end
	local stack_pct = self:GetAbility():GetSpecialValueFor( "buff_block" )
	-- get stronger value
...
function modifier_Middle_shield_crash:DeclareFunctions()
	local funcs = {
...
function modifier_Middle_shield_crash:OnTooltip()
    return self:Advanced_GetModifierTotalBlockConstantMaximum()
end



-- function modifier_Middle_shield_crash:GetModifierTotal_ConstantBlock()	return self:GetStackCount() end


function modifier_Middle_shield_crash:ADDeclareFunctions()
	return {
		advanced_MODIFIER_PROPERTY_TOTALBLOCK_CONSTANT_MAXIMUM
	}
end
function modifier_Middle_shield_crash:Advanced_GetModifierTotalBlockConstantMaximum(keys)
...
function modifier_Middle_shield_crash:GetStatusEffectName()
	return "particles/status_fx/status_effect_pangolier_shield.vpcf"
end

function modifier_Middle_shield_crash:StatusEffectPriority()	return MODIFIER_PRIORITY_NORMAL end
function modifier_Middle_shield_crash:PlayEffects()
...
modifier_Middle_shield_crash_debuff = modifier_Middle_shield_crash_debuff or class({})

function modifier_Middle_shield_crash_debuff:IsDebuff()			return true end
function modifier_Middle_shield_crash_debuff:IsHidden() 			return false end
function modifier_Middle_shield_crash_debuff:IsPurgable() 			return true end
function modifier_Middle_shield_crash_debuff:OnCreated()
	self.move_slow = -self:GetAbility():GetSpecialValueFor("debuff_slow")
end
function modifier_Middle_shield_crash_debuff:DeclareFunctions() return {
	MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT,

	} 
end

function modifier_Middle_shield_crash_debuff:GetModifierMoveSpeedBonus_Constant() return self.move_slow end