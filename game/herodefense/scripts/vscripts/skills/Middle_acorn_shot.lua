...
Middle_acorn_shot = Middle_acorn_shot or class({})
LinkLuaModifier( "modifier_Middle_acorn_shot", "skills/Middle_acorn_shot", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Middle_acorn_shot_thinker", "skills/Middle_acorn_shot", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Middle_acorn_shot_debuff", "skills/Middle_acorn_shot", LUA_MODIFIER_MOTION_NONE )

function Middle_acorn_shot:Precache( context )
...
function Middle_acorn_shot:Spawn()
	if not IsServer() then return end
end


function Middle_acorn_shot:GetCastRange( vLocation, hTarget )
	return self:GetCaster():Script_GetAttackRange() + self:GetSpecialValueFor( "bonus_range" )
end

function Middle_acorn_shot:OnSpellStart()
	-- unit identifier
	...
	local thinker = CreateModifierThinker(
		...
		self, -- ability source
		"modifier_Middle_acorn_shot_thinker", -- modifier name
		{ duration = 30 }, -- kv
		caster:GetOrigin(),
		caster:GetTeamNumber(),
		false
	)
	local mod = thinker:FindModifierByName( "modifier_Middle_acorn_shot_thinker" )
	if mod then
...
function Middle_acorn_shot:OnProjectileHit_ExtraData( target, location, ExtraData )
	local caster = self:GetCaster()
	...
	local mod = caster:AddNewModifier(
		caster, -- player source
		self, -- ability source
		"modifier_Middle_acorn_shot", -- modifier name
		{duration = 0.1} -- kv
	)
	local modifier_keys = {
		duration = 0.1,
	...
	if not target:IsMagicImmune() then
		...
		if duration>0 then
			target:AddNewModifier(
				caster, -- player source
				self, -- ability source
				"modifier_Middle_acorn_shot_debuff", -- modifier name
...
modifier_Middle_acorn_shot_thinker = modifier_Middle_acorn_shot_thinker or  class({})
function modifier_Middle_acorn_shot_thinker:OnCreated( kv )
	...
	local bonus_bounces = math.min(self.caster:GetDamageMax()/75,6)
...
function modifier_Middle_acorn_shot_thinker:InitCast( caster,target )
	self.source = caster
	self.target = target
end


function modifier_Middle_acorn_shot_thinker:OnDestroy()
	if not IsServer() then return end
	UTIL_Remove( self:GetParent() )
end

function modifier_Middle_acorn_shot_thinker:OnIntervalThink()
...
function modifier_Middle_acorn_shot_thinker:Bounce()
...
modifier_Middle_acorn_shot = modifier_Middle_acorn_shot or  class({})

function modifier_Middle_acorn_shot:IsHidden()	return true end
function modifier_Middle_acorn_shot:IsPurgable()	return false end
function modifier_Middle_acorn_shot:OnCreated( kv )
	if not IsServer() then return end
	self.bonus = self:GetAbility():GetSpecialValueFor( "bonus_damage" )
end
function modifier_Middle_acorn_shot:DeclareFunctions()
	local funcs = {
...
function modifier_Middle_acorn_shot:GetModifierPreAttack_BonusDamage()
	return self.bonus
end

-- function modifier_Middle_acorn_shot:GetModifierProcAttack_Feedback( params )
...
modifier_Middle_acorn_shot_debuff = modifier_Middle_acorn_shot_debuff or  class({})
function modifier_Middle_acorn_shot_debuff:IsHidden()	return false end
function modifier_Middle_acorn_shot_debuff:IsDebuff()	return true end
function modifier_Middle_acorn_shot_debuff:IsStunDebuff()	return false end
function modifier_Middle_acorn_shot_debuff:IsPurgable()	return true end
function modifier_Middle_acorn_shot_debuff:OnCreated( kv )
	self.slow = -self:GetAbility():GetSpecialValueFor( "slow" )
	if not IsServer() then return end
end

function modifier_Middle_acorn_shot_debuff:OnRefresh( kv )
	self:OnCreated( kv )
end


function modifier_Middle_acorn_shot_debuff:DeclareFunctions()
	local funcs = {
		MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT,
	}
	return funcs
end
function modifier_Middle_acorn_shot_debuff:GetModifierMoveSpeedBonus_Constant()	return self.slow end
function modifier_Middle_acorn_shot_debuff:GetEffectName()
	return "particles/units/heroes/hero_hoodwink/hoodwink_acorn_shot_slow.vpcf"
end
function modifier_Middle_acorn_shot_debuff:GetEffectAttachType()	return PATTACH_ABSORIGIN_FOLLOW end