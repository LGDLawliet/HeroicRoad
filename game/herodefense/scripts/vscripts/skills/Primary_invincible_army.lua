...
Primary_invincible_army = class({})
require("internal/timers")
LinkLuaModifier("modifier_Primary_invincible_army_thinker", "skills/Primary_invincible_army", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_invincible_army_triger", "skills/Primary_invincible_army", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_invincible_army_kill", "skills/Primary_invincible_army", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_invincible_army_damage", "skills/Primary_invincible_army", LUA_MODIFIER_MOTION_NONE)

function Primary_invincible_army:Precache( context )
...
function Primary_invincible_army:IsHiddenWhenStolen() 	return false end
function Primary_invincible_army:IsRefreshable() 		return true end
function Primary_invincible_army:IsStealable() 			return true end
function Primary_invincible_army:GetAOERadius()
	return self:GetSpecialValueFor("radius")
end
function Primary_invincible_army:OnSpellStart()
	...
	local thinker =CreateModifierThinker(
		self:GetCaster(),
		self,
		"modifier_Primary_invincible_army_thinker",
		{
...
function Primary_invincible_army:OnProjectileHit_ExtraData(target, location, keys)
	-- print("aaa")
...
modifier_Primary_invincible_army_thinker = class({})

function modifier_Primary_invincible_army_thinker:OnCreated(params)
	if IsServer() then
...
function modifier_Primary_invincible_army_thinker:OnIntervalThink()
	...
	Timers:CreateTimer(3, function()
		...
		if caster and not caster:IsNull() and ability and not ability:IsNull() then
			...
			if illusion then
				...
				illusion:AddNewModifier(caster, ability, "modifier_Primary_invincible_army_triger", {})
...
function modifier_Primary_invincible_army_thinker:OnDestroy(params)
...
modifier_Primary_invincible_army_triger = advanced_modifier({})

function modifier_Primary_invincible_army_triger:IsDebuff()			return false end
function modifier_Primary_invincible_army_triger:IsHidden() 			return false end
function modifier_Primary_invincible_army_triger:IsPurgable() 		return false end
function modifier_Primary_invincible_army_triger:IsPurgeException() 	return false end
function modifier_Primary_invincible_army_triger:GetStatusEffectName() return "particles/status_fx/status_effect_techies_tazer.vpcf" end
function modifier_Primary_invincible_army_triger:StatusEffectPriority() return 100000 end
function modifier_Primary_invincible_army_triger:CheckState() return 
...
function modifier_Primary_invincible_army_triger:DeclareFunctions() return {
...
function modifier_Primary_invincible_army_triger:OnCreated(keys)
	if IsServer() then
		-- self.damageTable = {
        --     -- victim 			= enemy,
...
function modifier_Primary_invincible_army_triger:Advanced_GetModifierAttackRangeOverride() return self.radius end
function modifier_Primary_invincible_army_triger:GetModifierAttackSpeedBaseOverride() return self.attack_speed end
function modifier_Primary_invincible_army_triger:GetAttackSound() return self.sound  end
-- function modifier_Primary_invincible_army_triger:GetModifierTotalDamageOutgoing_Percentage() return -100  end
function modifier_Primary_invincible_army_triger:GetActivityTranslationModifiers()	
...
function modifier_Primary_invincible_army_triger:OnIntervalThink()
	if not IsServer() then
		return
	end
	local caster = self:GetCaster()  
...
function modifier_Primary_invincible_army_triger:OnAttackLanded(keys)
	...
	target:AddNewModifier(caster, ability, "modifier_Primary_invincible_army_damage", {stack = damage})
...
-- advanced_modifier
function modifier_Primary_invincible_army_triger:ADDeclareFunctions()
	local funcs = {
...
function modifier_Primary_invincible_army_triger:Advanced_GetModifierTotalDamageOutgoing_Percentage(keys)
    return -100
end
...
modifier_invincible_army_kill = class({})

function modifier_invincible_army_kill:IsDebuff()			return false end
function modifier_invincible_army_kill:IsHidden() 			return false end
function modifier_invincible_army_kill:IsPurgable() 		return false end
function modifier_invincible_army_kill:IsPurgeException() 	return false end
function modifier_invincible_army_kill:OnDestroy()
	if IsServer() then
		local parent = self:GetParent()
		...
		local pfx = ParticleManager:CreateParticle( "particles/econ/events/pw_compendium_2014/teleport_start_l_pw2014.vpcf", PATTACH_WORLDORIGIN, self:GetCaster() )
...
modifier_Primary_invincible_army_damage = class({})

function modifier_Primary_invincible_army_damage:IsDebuff() return true end
function modifier_Primary_invincible_army_damage:IsHidden() return false end
function modifier_Primary_invincible_army_damage:IsPurgable() return false end
function modifier_Primary_invincible_army_damage:IsPurgeException() return false end
function modifier_Primary_invincible_army_damage:OnCreated(keys)
	if IsServer() then
...
function modifier_Primary_invincible_army_damage:OnRefresh(keys)
	if IsServer() then
		self:SetStackCount(self:GetStackCount()+ keys.stack)

	end
end
function modifier_Primary_invincible_army_damage:OnIntervalThink()
	local ability = self:GetAbility()
	if not ability then
		self:SafeDestroy()
		return
