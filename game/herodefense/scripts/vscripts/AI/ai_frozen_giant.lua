...
function Spawn( entityKeyValues )
	...
	Timers:CreateTimer(0.05, function()
		...
		if thisEntity.challenge_level then
			...
			unit:SetBaseDamageMax(150+20*_G.GAME_ROUND*level)
			unit:SetBaseDamageMin(150+20*_G.GAME_ROUND*level)
			SetCreatureHealth(unit, 3000+900*_G.GAME_ROUND*level, true)
			unit:SetPhysicalArmorBaseValue(15+level*6)
		elseif thisEntity.endlessLevel then
			local level = thisEntity.endlessLevel
			local unit = thisEntity
			unit:SetBaseDamageMax(3500+1600*level*(1+level*0.04))
			unit:SetBaseDamageMin(3500+1600*level*(1+level*0.04))
			SetCreatureHealth(unit, 55000+55000*level*(1+level*0.04), true)
