Primary_timber_chain = class({})

require("internal/timers")
LinkLuaModifier( "modifier_Primary_timber_chain", "skills/Primary_timber_chain", LUA_MODIFIER_MOTION_HORIZONTAL )

function Primary_timber_chain:Precache( context )
...
function Primary_timber_chain:GetCastRange(vLocation, hTarget)
	-- if IsServer() then return 900000 end
	return self:GetSpecialValueFor( "range" )
end
function Primary_timber_chain:OnSpellStart()
	-- unit identifier
...
function Primary_timber_chain:Spawn()
	self.projectiles = {}
end


function Primary_timber_chain:OnProjectileThinkHandle( handle )
	-- get data
	...
	if CalculateDistance(target,location)<=250 then
		...
		if ability and ability:GetAutoCastState() then
			...
			target:AddNewModifier(
					...
					self, -- ability source
					"modifier_Primary_timber_chain", -- modifier name
					{
						duration = 3,
						point_x = point.x,
						point_y = point.y,
...
function Primary_timber_chain:OnProjectileHitHandle( target, location, handle )
	local ExtraData = self.projectiles[ handle ]
...
function Primary_timber_chain:PlayEffects( point, speed, duration )
	-- Get Resources
...
function Primary_timber_chain:ModifyEffects1( effect )
	-- retract
	ParticleManager:SetParticleControlEnt(
		effect,
		1,
...
function Primary_timber_chain:ModifyEffects2( effect, point )
...
function Primary_timber_chain:ModifyEffectsTalent(effect,target)
	ParticleManager:SetParticleControlEnt(
		effect,
		1,
		target,
...
modifier_Primary_timber_chain = class({})


function modifier_Primary_timber_chain:IsHidden()	return true end
function modifier_Primary_timber_chain:IsDebuff()	return false end
function modifier_Primary_timber_chain:IsStunDebuff()	return false end
function modifier_Primary_timber_chain:IsPurgable()	return false end
function modifier_Primary_timber_chain:OnCreated( kv )
...
function modifier_Primary_timber_chain:OnRefresh( kv )
	if not IsServer() then return end
...
function modifier_Primary_timber_chain:OnRemoved()
end

function modifier_Primary_timber_chain:OnDestroy()
	if not IsServer() then return end
...
--------------------------------------------------------------------------------
-- Status Effects
function modifier_Primary_timber_chain:CheckState()
	local state = {
...
--------------------------------------------------------------------------------
-- Motion Effects
function modifier_Primary_timber_chain:UpdateHorizontalMotion( me, dt )
...
function modifier_Primary_timber_chain:OnHorizontalMotionInterrupted()
	-- destroy
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_Primary_timber_chain:PlayEffects( target )
	-- Get Resources
