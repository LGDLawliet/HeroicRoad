LinkLuaModifier("modifier_Primary_Einherjar", "skills/Primary_Einherjar", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_Einherjar_buff", "skills/Primary_Einherjar", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_Einherjar_ghost", "skills/Primary_Einherjar", LUA_MODIFIER_MOTION_NONE)


require('internal/timers')   --计时器功能
--Abilities
if Primary_Einherjar == nil then
	Primary_Einherjar = class({})
end

function Primary_Einherjar:GetCastRange()
	return self:GetSpecialValueFor("radius")
end
function Primary_Einherjar:OnSpellStart()
	local hCaster = self:GetCaster()
	local duration = self:GetSpecialValueFor("duration")

	hCaster:AddNewModifier(hCaster, self, "modifier_Primary_Einherjar_buff", { duration = duration })
...
function Primary_Einherjar:GetIntrinsicModifierName()
	return "modifier_Primary_Einherjar"
end

---------------------------------------------------------------------
--Modifiers
modifier_Primary_Einherjar = modifier_Primary_Einherjar or class({})
function modifier_Primary_Einherjar:IsHidden() return true end
function modifier_Primary_Einherjar:IsDebuff() return false end
function modifier_Primary_Einherjar:IsPurgable() return false end
function modifier_Primary_Einherjar:IsPurgeException() return false end
function modifier_Primary_Einherjar:IsStunDebuff() return false end
function modifier_Primary_Einherjar:AllowIllusionDuplicate() return false end
function modifier_Primary_Einherjar:OnCreated(params)
	if IsServer() then
		self.tScepterGhosts = {}
		self:StartIntervalThink(0.25)
	end
end
function modifier_Primary_Einherjar:OnRefresh(params)
end
function modifier_Primary_Einherjar:OnDestroy()
	if IsServer() then
		for n, hGhost in pairs(self.tScepterGhosts) do
			if IsValid(hGhost) then
				hGhost:RemoveModifierByName("modifier_Primary_Einherjar_ghost")
			end
		end
	end
end
function modifier_Primary_Einherjar:OnIntervalThink()
	if IsServer() then
...
---------------------------------------------------------------------
modifier_Primary_Einherjar_buff =modifier_Primary_Einherjar_buff  or class({})
function modifier_Primary_Einherjar_buff:IsHidden()	return false end
function modifier_Primary_Einherjar_buff:IsDebuff()	return false end
function modifier_Primary_Einherjar_buff:IsPurgable()	return false end
function modifier_Primary_Einherjar_buff:IsPurgeException()	return false end
function modifier_Primary_Einherjar_buff:IsStunDebuff()	return false end
function modifier_Primary_Einherjar_buff:AllowIllusionDuplicate()	return false end
function modifier_Primary_Einherjar_buff:DestroyOnExpire()	return false end
-- function modifier_Primary_Einherjar_buff:GetAttributes()
-- 	return MODIFIER_ATTRIBUTE_MULTIPLE
-- end
function modifier_Primary_Einherjar_buff:OnCreated(params)
	...
	if IsServer() then
		...
		self.unique_str = DoUniqueString("modifier_Primary_Einherjar_buff")

		self.attack_cooldown = 0.7  -- 设置攻击冷却时间为1秒
					...
					if hGhost.hUnit:IsPositionInRange(vTargetPosition, 100) then
						if hGhost.hTarget ~= nil then
							if hGhost.bReturning then
								...
								if self:GetRemainingTime() <= 0 then
									hGhost.hUnit:RemoveModifierByName("modifier_Primary_Einherjar_ghost")
...
function modifier_Primary_Einherjar_buff:OnDestroy()
	if IsServer() then
		local hParent = self:GetParent()
...
function modifier_Primary_Einherjar_buff:OnIntervalThink()
	if IsServer() then
		local hAbility = self:GetAbility()
		local hCaster = self:GetCaster()
		local hParent = self:GetParent()
		...
		if #(self.tGhosts) < self.spirits then
			...
			local hGhost = {
				hUnit = CreateModifierThinker(hCaster, hAbility, "modifier_Primary_Einherjar_ghost", nil, vPosition, hCaster:GetTeamNumber(), false),
				vTargetPosition = nil,
				hTarget = nil,
				bReturning = false
			}
...
---------------------------------------------------------------------
modifier_Primary_Einherjar_ghost = modifier_Primary_Einherjar_ghost or class({})
function modifier_Primary_Einherjar_ghost:IsHidden()	return true end
function modifier_Primary_Einherjar_ghost:IsDebuff()	return false end
function modifier_Primary_Einherjar_ghost:IsPurgable()	return false end
function modifier_Primary_Einherjar_ghost:IsPurgeException()	return false end
function modifier_Primary_Einherjar_ghost:IsStunDebuff()	return false end
function modifier_Primary_Einherjar_ghost:AllowIllusionDuplicate()	return false end
function modifier_Primary_Einherjar_ghost:OnCreated(params)
	if IsServer() then

		self:GetParent():SetModelScale(0.9)
	end
end
function modifier_Primary_Einherjar_ghost:OnDestroy()
	if IsServer() then
...
function modifier_Primary_Einherjar_ghost:CheckState()
	return {
...
function modifier_Primary_Einherjar_ghost:DeclareFunctions()
	return {
...
function modifier_Primary_Einherjar_ghost:GetModifierModelChange(params)
...
function modifier_Primary_Einherjar_ghost:GetOverrideAnimation(params)
...
function modifier_Primary_Einherjar_ghost:GetStatusEffectName() return "particles/new_effect/status/new_status_effect_soul_02.vpcf" end
function modifier_Primary_Einherjar_ghost:GetEffectAttachType() return PATTACH_ABSORIGIN_FOLLOW end
---------------------------------------------------------------------
