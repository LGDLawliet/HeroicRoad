...
LinkLuaModifier("modifier_Primary_Moment_of_Courage_buff", "skills/Primary_Moment_of_Courage", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_Moment_of_Courage_active", "skills/Primary_Moment_of_Courage", LUA_MODIFIER_MOTION_NONE)

Primary_Moment_of_Courage	= Primary_Moment_of_Courage or class({})
require("internal/timers")

function Primary_Moment_of_Courage:Precache( context )
	PrecacheResource( "particle", "particles/units/heroes/hero_legion_commander/legion_commander_courage_hit.vpcf", context )
end
function Primary_Moment_of_Courage:GetIntrinsicModifierName()
	return "modifier_Primary_Moment_of_Courage_buff"
end
-----------------------------------------------------------------------------------------------------------------

modifier_Primary_Moment_of_Courage_buff = advanced_modifier({})
function modifier_Primary_Moment_of_Courage_buff:IsDebuff()	return false end
function modifier_Primary_Moment_of_Courage_buff:IsHidden()	return true end
function modifier_Primary_Moment_of_Courage_buff:IsPurgable()	return false end

function modifier_Primary_Moment_of_Courage_buff:OnCreated()
...
function modifier_Primary_Moment_of_Courage_buff:OnRefresh()
	self.chance = self:GetAbility():GetSpecialValueFor("chance")
	if IsServer() then
...
function modifier_Primary_Moment_of_Courage_buff:ADDeclareFunctions()	
	return {
...
function modifier_Primary_Moment_of_Courage_buff:DeclareFunctions()	
	return {
		MODIFIER_PROPERTY_EVASION_CONSTANT
		}
end

function modifier_Primary_Moment_of_Courage_buff:GetModifierEvasion_Constant()
...
function modifier_Primary_Moment_of_Courage_buff:Advanced_GetModifierIncomingDamage_Percentage(keys)
...
function modifier_Primary_Moment_of_Courage_buff:OnAttackLanded(keys)
	if not IsServer() then
		return 
	end
    ...
    if  self:GetAbility():IsCooldownReady() then
		...
		if self.chance >= random(1,100) then
			caster:AddNewModifier(caster, self:GetAbility(), "modifier_Primary_Moment_of_Courage_active", {duration = 2})
	    end
    end
end
...
modifier_Primary_Moment_of_Courage_active = advanced_modifier({})

function modifier_Primary_Moment_of_Courage_active:IsDebuff()	return false end
function modifier_Primary_Moment_of_Courage_active:IsHidden()	return true end
function modifier_Primary_Moment_of_Courage_active:IsPurgable() return false end
function modifier_Primary_Moment_of_Courage_active:IsPurgeException() return false end
function modifier_Primary_Moment_of_Courage_active:ADDeclareFunctions()	return 
	{
		MODIFIER_EVENT_ON_ATTACK_LANDED = {self:GetParent(),nil},
		MODIFIER_EVENT_ON_TAKEDAMAGE = {self:GetParent(),nil},
	} 
end
function modifier_Primary_Moment_of_Courage_active:OnCreated(keys)
	if IsServer() then
		self.active = false
	end
end
function modifier_Primary_Moment_of_Courage_active:OnAttackLanded(keys)
	if not IsServer() then
		return 
	end
	local caster = self:GetCaster()
...
function modifier_Primary_Moment_of_Courage_active:OnTakeDamage( params )
