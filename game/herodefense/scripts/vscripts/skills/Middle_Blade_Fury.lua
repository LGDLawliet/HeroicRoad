...
LinkLuaModifier( "modifier_Middle_Blade_Fury", "skills/Middle_Blade_Fury", LUA_MODIFIER_MOTION_NONE )

Middle_Blade_Fury = class({})

--------------------------------------------------------------------------------
function Middle_Blade_Fury:GetCastRange()
	return self:GetSpecialValueFor("radius") - self:GetCaster():GetCastRangeBonus()
end
function Middle_Blade_Fury:OnSpellStart()
	...
	caster:AddNewModifier(
		caster,
		self, 
		"modifier_Middle_Blade_Fury",
		{ duration = bDuration }
	)
end
-------------------------------------------------------------------------------
modifier_Middle_Blade_Fury = advanced_modifier({})


function modifier_Middle_Blade_Fury:IsHidden()	return false end
function modifier_Middle_Blade_Fury:IsDebuff()	return false end
function modifier_Middle_Blade_Fury:IsPurgable()	return false end

function modifier_Middle_Blade_Fury:OnCreated( kv )
	if IsServer() then
...
function modifier_Middle_Blade_Fury:OnRefresh( kv )
	if IsServer() then
		self.tick = self:GetAbility():GetSpecialValueFor( "interval" )
		self.radius = self:GetAbility():GetSpecialValueFor( "radius" )
		self.max = self:GetAbility():GetSpecialValueFor( "max" )
...
function modifier_Middle_Blade_Fury:OnDestroy( kv )
	local sound_cast = "Hero_Juggernaut.BladeFuryStart"
	StopSoundOn( sound_cast, self:GetParent() )
end

function modifier_Middle_Blade_Fury:OnIntervalThink()
	self.max = self:GetAbility():GetSpecialValueFor( "max" )
	self:Fury(self.radius,self.max)
end

-- 单次风暴执行
function modifier_Middle_Blade_Fury:Fury(radius,max)
...
function modifier_Middle_Blade_Fury:CheckState()
	local state = {
...
function modifier_Middle_Blade_Fury:DeclareFunctions()
	return{
		MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT,
	}
end
function modifier_Middle_Blade_Fury:GetModifierMoveSpeedBonus_Constant()
	return self:GetAbility():GetSpecialValueFor("bonus_move")
end

function modifier_Middle_Blade_Fury:PlayEffects()
...
function modifier_Middle_Blade_Fury:PlayEffects2( target )
