...
function Spawn( entityKeyValues )
	...
	Ability1 = thisEntity:FindAbilityByName( "leshrac_challenge_split_earth" )
	...
	Timers:CreateTimer(0.05, function()
		if thisEntity.challenge_level then
			...
			unit:SetBaseDamageMax(100+10*_G.GAME_ROUND*level)
			unit:SetBaseDamageMin(100+10*_G.GAME_ROUND*level)
			SetCreatureHealth(unit, 2700+900*_G.GAME_ROUND*level, true)
			unit:SetPhysicalArmorBaseValue(5+level*5)
		elseif thisEntity.endlessLevel then
			local level = thisEntity.endlessLevel
			local unit = thisEntity
			unit:SetBaseDamageMax(1200+600*level*(1+level*0.04))
			unit:SetBaseDamageMin(1200+600*level*(1+level*0.04))
			SetCreatureHealth(unit, 70000+60000*level*(1+level*0.04), true)
...
function AIThink()
    ...
    if Ability1 ~= nil and Ability1:IsFullyCastable() then
		local enemies = FindUnitsInRadius( thisEntity:GetTeamNumber(), thisEntity:GetOrigin(), nil, 700, 
