...
LinkLuaModifier("modifier_Middle_Arc_Lightning", "skills/Middle_Arc_Lightning", LUA_MODIFIER_MOTION_NONE)

Middle_Arc_Lightning			= Middle_Arc_Lightning or class({})

function Middle_Arc_Lightning:Precache( context )
...
function Middle_Arc_Lightning:GetAbilityTextureName()
...
function Middle_Arc_Lightning:OnSpellStart()
	...
	if not target:TriggerSpellAbsorb(self) then
		...
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_Middle_Arc_Lightning", {
...
modifier_Middle_Arc_Lightning= modifier_Middle_Arc_Lightning or class({})

function modifier_Middle_Arc_Lightning:IsHidden()		return true end
function modifier_Middle_Arc_Lightning:IsPurgable()		return false end
function modifier_Middle_Arc_Lightning:RemoveOnDeath()	return false end
function modifier_Middle_Arc_Lightning:GetAttributes()	return MODIFIER_ATTRIBUTE_MULTIPLE end

function modifier_Middle_Arc_Lightning:OnCreated(keys)
	...
	if self.current_unit  and not self.current_unit:IsNull() then  
		...
		-- self.current_unit						= EntIndexToHScript(self.starting_unit_entindex)
		if not (RandomInt(1, 100)>12) then
...
function modifier_Middle_Arc_Lightning:OnIntervalThink()
