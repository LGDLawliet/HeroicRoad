...
LinkLuaModifier("modifier_Middle_false_promise_timer", "skills/Middle_false_promise", LUA_MODIFIER_MOTION_NONE)

Middle_false_promise	= Middle_false_promise or class({})



function Middle_false_promise:IsRefreshable() return false end

function Middle_false_promise:OnSpellStart()
	...
	target:AddNewModifier(self:GetCaster(), self, "modifier_Middle_false_promise_timer", {duration = self:GetSpecialValueFor("duration")})

end



modifier_Middle_false_promise_timer	= modifier_Middle_false_promise_timer or advanced_modifier({})

function modifier_Middle_false_promise_timer:GetPriority()	return MODIFIER_PRIORITY_ULTRA end
function modifier_Middle_false_promise_timer:IsPurgable()	return false end
function modifier_Middle_false_promise_timer:GetEffectName()	return "particles/units/heroes/hero_oracle/oracle_false_promise.vpcf" end

function modifier_Middle_false_promise_timer:OnCreated()
...
function modifier_Middle_false_promise_timer:OnRefresh()
	if not IsServer() then return end
...
function modifier_Middle_false_promise_timer:OnDestroy()
	if not IsServer() then return end
...
function modifier_Middle_false_promise_timer:DeclareFunctions()
	return {
...
function modifier_Middle_false_promise_timer:Advanced_GetModifierIncomingDamage_Percentage(keys)
	if IsClient() then
		return -100
	end
...
function modifier_Middle_false_promise_timer:OnHealReceived(keys)
	if keys.unit == self:GetParent() and self:GetRemainingTime() >= 0 then
		self.heal_counter = self.heal_counter + (keys.gain * 2.5)
...
function modifier_Middle_false_promise_timer:GetDisableHealing(keys)
	return 1
end

function modifier_Middle_false_promise_timer:ADDeclareFunctions()
