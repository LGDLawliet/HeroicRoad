...
LinkLuaModifier("modifier_Primary_gravity_thinker", "skills/Primary_gravity", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_gravity_damage", "skills/Primary_gravity", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_gravity_motion", "skills/Primary_gravity", LUA_MODIFIER_MOTION_NONE)
Primary_gravity = class ({})




function Primary_gravity:GetAOERadius()return self:GetSpecialValueFor("radius") end

function Primary_gravity:OnSpellStart()
	local caster = self:GetCaster()
	local point = self:GetCursorPosition()
	local duration = self:GetSpecialValueFor("duration")
	local thinker = CreateModifierThinker(caster, self, "modifier_Primary_gravity_thinker", {duration = duration,posx=point.x,posy=point.y}, self:GetCursorPosition(), caster:GetTeamNumber(), false)
	thinker:AddNewModifier(caster, self, "modifier_Primary_gravity_damage", {duration =duration})
end

modifier_Primary_gravity_thinker = class({})

function modifier_Primary_gravity_thinker:IsDebuff()			return true end
function modifier_Primary_gravity_thinker:IsHidden() 			return true end
function modifier_Primary_gravity_thinker:IsPurgable() 		return true end
function modifier_Primary_gravity_thinker:IsPurgeException() 	return true end
function modifier_Primary_gravity_thinker:OnCreated(keys)
	if IsServer() then
...
function modifier_Primary_gravity_thinker:OnIntervalThink()
	...
	for _, unit in pairs(enemies) do
		unit:AddNewModifier(self:GetParent(), ability, "modifier_Primary_gravity_motion", {duration = 0.6})
	end
end

function modifier_Primary_gravity_thinker:OnDestroy(keys)
	if IsServer() then
		...
		if self:GetAbility() then
			...
			DOTA_UNIT_TARGET_FLAG_NONE+DOTA_UNIT_TARGET_FLAG_NONE ,
...
modifier_Primary_gravity_damage = class({})

function modifier_Primary_gravity_damage:IsDebuff()			return true end
function modifier_Primary_gravity_damage:IsHidden() 			return true end
function modifier_Primary_gravity_damage:IsPurgable() 		return true end
function modifier_Primary_gravity_damage:IsPurgeException() 	return true end
function modifier_Primary_gravity_damage:OnCreated(keys)
	if IsServer() then
		local ability = self:GetAbility()
		self.damageTable = {
...
function modifier_Primary_gravity_damage:OnIntervalThink()

	if not self:GetAbility() or not self:GetAbility() then
		self:SafeDestroy()
		return
	end
	local enemies = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, self:GetAbility():GetSpecialValueFor("radius"),
...
modifier_Primary_gravity_motion = class({})

function modifier_Primary_gravity_motion:IsDebuff()			return true end
function modifier_Primary_gravity_motion:IsHidden() 			return true end
function modifier_Primary_gravity_motion:IsPurgable() 		return false end
function modifier_Primary_gravity_motion:IsPurgeException() 	return false end
function modifier_Primary_gravity_motion:IsMotionController() return true end

function modifier_Primary_gravity_motion:OnCreated(keys)
	if IsServer() then
...
function modifier_Primary_gravity_motion:OnDestroy()
	if IsServer() then
...
function modifier_Primary_gravity_motion:OnIntervalThink(keys)   
	if  IsServer() then
		...
		local dt = FrameTime()
		local new_pos = me:GetAbsOrigin() + direction * (200 / (1.0 / dt))  
