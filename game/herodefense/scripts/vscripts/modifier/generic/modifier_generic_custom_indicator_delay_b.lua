...
--------------------------------------------------------------------------------
modifier_generic_custom_indicator_delay_b = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_generic_custom_indicator_delay_b:IsHidden()
	return true
end

function modifier_generic_custom_indicator_delay_b:IsPurgable()
	return true
end

function modifier_generic_custom_indicator_delay_b:GetAttributes()
	return MODIFIER_ATTRIBUTE_PERMANENT + MODIFIER_ATTRIBUTE_MULTIPLE
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_generic_custom_indicator_delay_b:OnCreated( kv )
	if IsServer() then return end

	-- register modifier to ability
	self:GetAbility().custom_indicator_b = self
end

function modifier_generic_custom_indicator_delay_b:OnRefresh( kv )
end

function modifier_generic_custom_indicator_delay_b:OnRemoved()
end

function modifier_generic_custom_indicator_delay_b:OnDestroy()
end
...
function modifier_generic_custom_indicator_delay_b:OnIntervalThink()
	if IsClient() then
...
function modifier_generic_custom_indicator_delay_b:Register( loc,point ,delay)
...
function modifier_generic_custom_indicator_delay_b:Destroy()
	if self.init then
		self.init = nil
		ParticleManager:DestroyParticle( self.effect_cast, true ) --注意这里原版写了false 会延迟一小会儿销毁特效 
		ParticleManager:ReleaseParticleIndex( self.effect_cast )
