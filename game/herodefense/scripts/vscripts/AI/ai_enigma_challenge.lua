...
function Spawn( entityKeyValues )
	...
	Ability1 = thisEntity:FindAbilityByName( "enigma_challenge_Black_Hole" )
	...
	Timers:CreateTimer(0.05, function()
		...
		elseif thisEntity.endlessLevel then
			...
			local unit = thisEntity
			unit:SetBaseDamageMax(2500+1400*level*(1+level*0.04))
			unit:SetBaseDamageMin(2500+1400*level*(1+level*0.04))
...
function AIThink()
    ...
    if Ability1 ~= nil and Ability1:IsFullyCastable() then
		local enemies = FindUnitsInRadius( thisEntity:GetTeamNumber(), thisEntity:GetOrigin(), nil, 900,
