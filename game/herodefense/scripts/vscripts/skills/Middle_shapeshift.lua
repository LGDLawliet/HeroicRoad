...
Middle_shapeshift = class({})
LinkLuaModifier("modifier_Middle_shapeshift_transform_stun", "skills/Middle_shapeshift", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Middle_shapeshift_transform", "skills/Middle_shapeshift", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Middle_shapeshift", "skills/Middle_shapeshift", LUA_MODIFIER_MOTION_NONE)

function Middle_shapeshift:Precache( context )
...
function Middle_shapeshift:OnSpellStart()
	-- Ability properties
	...
	caster.Form_MODIFIER_NAME = "modifier_Middle_shapeshift_transform"
	...
	caster:AddNewModifier(caster, ability, "modifier_Middle_shapeshift_transform_stun", {duration = transformation_time})
	...
	Timers:CreateTimer(transformation_time, function()
		...
		caster:AddNewModifier(caster, ability, "modifier_Middle_shapeshift_transform", {duration = duration*gain})
	end)	
end


modifier_Middle_shapeshift_transform_stun = class({})

function modifier_Middle_shapeshift_transform_stun:CheckState()	
	local state = {[MODIFIER_STATE_STUNNED] = true}
	return state	
end
function modifier_Middle_shapeshift_transform_stun:IsHidden()
	return true
end

modifier_Middle_shapeshift_transform = advanced_modifier({})
function modifier_Middle_shapeshift_transform:IsHidden()	return false end
function modifier_Middle_shapeshift_transform:IsPurgable()	return false end
function modifier_Middle_shapeshift_transform:IsDebuff()	return false end
function modifier_Middle_shapeshift_transform:IsAura() return true end
function modifier_Middle_shapeshift_transform:GetAuraDuration() return 0.1 end
function modifier_Middle_shapeshift_transform:GetModifierAura() return "modifier_Middle_shapeshift" end
function modifier_Middle_shapeshift_transform:GetAuraRadius() return 4000 end
function modifier_Middle_shapeshift_transform:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_PLAYER_CONTROLLED end
function modifier_Middle_shapeshift_transform:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function modifier_Middle_shapeshift_transform:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
function modifier_Middle_shapeshift_transform:AllowIllusionDuplicate()	return false end
function modifier_Middle_shapeshift_transform:GetAuraEntityReject(target)
    if IsServer() then	    	
...
function modifier_Middle_shapeshift_transform:DeclareFunctions()	
		local decFuncs = {
...
function modifier_Middle_shapeshift_transform:GetModifierModelScale() 
    return 25
end


function modifier_Middle_shapeshift_transform:GetModifierModelChange()
...
function modifier_Middle_shapeshift_transform:OnCreated()
    if IsServer() then
...
function modifier_Middle_shapeshift_transform:OnDestroy()
    if IsServer() then    	
...
function modifier_Middle_shapeshift_transform:OnTakeDamage(tg)
    if IsServer() then   
		...
		if tg.attacker==parent 	and not parent:IsIllusion() and tg.damage_category==DOTA_DAMAGE_CATEGORY_ATTACK  then 
			...
			local hp = tg.damage*0.1*life_steal_gain
...
function modifier_Middle_shapeshift_transform:GetModifierAttackSpeedBonus_Constant() 	return self:GetStackCount()*10 end

function modifier_Middle_shapeshift_transform:Advanced_GetModifierAttackRangeOverride() 	return 200 end

function modifier_Middle_shapeshift_transform:ADDeclareFunctions()
	return {
...
-- Speed/crit modifier
modifier_Middle_shapeshift = advanced_modifier({})
function modifier_Middle_shapeshift:IsHidden()	return true end
function modifier_Middle_shapeshift:IsPurgable()	return false end
function modifier_Middle_shapeshift:IsDebuff()	return false end
function modifier_Middle_shapeshift:OnCreated()	
    -- Ability properties
    self.caster = self:GetCaster()
    self.ability = self:GetAbility()
	self.parent = self:GetParent()    
	self.certain_crit_buff = "modifier_Middle_shapeshift_certain_crit"
    self.transform_buff = "modifier_Middle_shapeshift_transform"
...
function modifier_Middle_shapeshift:GetEffectName()
	return "particles/units/heroes/hero_lycan/lycan_shapeshift_buff.vpcf"
end

function modifier_Middle_shapeshift:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_Middle_shapeshift:DeclareFunctions()
		local decFuncs = {
...
function modifier_Middle_shapeshift:Advanced_GetBonusNightVision()	
	return self.night_vision_bonus
end

function modifier_Middle_shapeshift:GetModifierMoveSpeed_AbsoluteMin()
...
-- advanced_modifier
function modifier_Middle_shapeshift:ADDeclareFunctions()
	local funcs = {
...
function modifier_Middle_shapeshift:Advanced_GetModifierTotalDamageOutgoing_Percentage(keys)
	if IsServer() then
