Primary_Holy_Light_Shield = class({})

LinkLuaModifier("modifier_primary_Holy_light_shield", "skills/Primary_Holy_light_shield", LUA_MODIFIER_MOTION_NONE)

function Primary_Holy_Light_Shield:Precache( context )
...
function Primary_Holy_Light_Shield:OnSpellStart()
    local target = self:GetCaster()
    local buff = target:FindModifierByName("modifier_primary_Holy_light_shield")
    if buff then
        buff:SetStackCount(0)
        buff:SafeDestroy()
    end
    local modifierStatusgain = self:GetCaster():GetModifierDurationGainIndex(1)
    target:AddNewModifier(self:GetCaster(),self,"modifier_primary_Holy_light_shield",{duration = self:GetSpecialValueFor("duration")*modifierStatusgain})
    target:Purge(false, true, false, true, false)
end

modifier_primary_Holy_light_shield = advanced_modifier({})

function modifier_primary_Holy_light_shield:IsDebuff()          return false end
function modifier_primary_Holy_light_shield:IsHidden()          return false end
function modifier_primary_Holy_light_shield:IsPurgable()        return false end
function modifier_primary_Holy_light_shield:IsPurgeException()  return false end
function modifier_primary_Holy_light_shield:OnCreated()
    if IsServer() then
...
function modifier_primary_Holy_light_shield:OnDestroy()
    if IsServer() then
...
function modifier_primary_Holy_light_shield:ADDeclareFunctions()
	return {
...
function modifier_primary_Holy_light_shield:AdvancedGetModifierTotal_ConstantBlock(keys)
