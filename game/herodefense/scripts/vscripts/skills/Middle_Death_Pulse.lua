Middle_Death_Pulse = Middle_Death_Pulse or class({})

function Middle_Death_Pulse:GetCastRange(vLocation, hTarget)
	return self:GetSpecialValueFor("radius") - self:GetCaster():GetCastRangeBonus()
end

function Middle_Death_Pulse:OnSpellStart()
...
function Middle_Death_Pulse:ReleaseProjectile(srouce,target,state)
	local projectile =
		{
			Target = target,
			Source = srouce,
...
function Middle_Death_Pulse:OnProjectileHit_ExtraData(target, vLocation, extraData)
	if IsServer() then
		local caster = self:GetCaster()
