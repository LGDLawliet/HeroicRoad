...
LinkLuaModifier("modifier_Middle_infest", "skills/Middle_infest", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Middle_infest_effect", "skills/Middle_infest", LUA_MODIFIER_MOTION_NONE)

Middle_infest = class ({})

function Middle_infest:GetAbilityTargetTeam()
	return DOTA_UNIT_TARGET_TEAM_FRIENDLY
end

function Middle_infest:CastFilterResultTarget(target)
...
function Middle_infest:GetCastRange()
	local caster = self:GetCaster()
	return 150 - caster:GetCastRangeBonus()

end

function Middle_infest:OnAbilityPhaseStart()
...
function Middle_infest:OnSpellStart()
	local target = self:GetCursorTarget()
	...
	local modifier = self:GetCaster():FindModifierByName("modifier_Middle_infest")
	if modifier then
	...
	local infest_modifier = self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_Middle_infest", 
	{
		duration =duration,
		target_ent		= target:entindex(),
	})
	
	local infest_effect_modifier = target:AddNewModifier(self:GetCaster(), self, "modifier_Middle_infest_effect", {duration=duration})
...
-- INFEST MODIFIER --
---------------------
modifier_Middle_infest = advanced_modifier ({})
function modifier_Middle_infest:IsHidden()	return true end
function modifier_Middle_infest:IsPurgable()	return false end
function modifier_Middle_infest:GetPriority()
	return 20
end

function modifier_Middle_infest:OnCreated(params)
...
function modifier_Middle_infest:OnIntervalThink()
	local ability = self:GetAbility()
...
function modifier_Middle_infest:OnDestroy()
	if not IsServer() then return end
...
function modifier_Middle_infest:ADDeclareFunctions()
	return {
...
function modifier_Middle_infest:AdvancedGetModifierConstantHealthRegenAmpPercentage()
	return -self.regen_down
end

function modifier_Middle_infest:AdvancedGetModifierConstantManaRegenAmpPercentage()
	return -self.regen_down
end

function modifier_Middle_infest:DeclareFunctions()
	return {
		MODIFIER_EVENT_ON_ORDER,
	}
end

function modifier_Middle_infest:OnOrder(keys)
	if not IsServer() then return end
...
function modifier_Middle_infest:CheckState(keys)
	if not IsServer() then return end
...
----------------------------
modifier_Middle_infest_effect = advanced_modifier ({})

function modifier_Middle_infest_effect:IsHidden()		return false end
function modifier_Middle_infest_effect:IsPurgable()		return false end
function modifier_Middle_infest_effect:GetAttributes()			return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_Middle_infest_effect:ShouldUseOverheadOffset() return true end

function modifier_Middle_infest_effect:OnCreated()
	local ability = self:GetAbility()
	self.radius	= ability:GetSpecialValueFor("radius")
...
function modifier_Middle_infest_effect:OnDestroy()
	if not IsServer() then return end
...
function modifier_Middle_infest_effect:ADDeclareFunctions()
	return {
...
function modifier_Middle_infest_effect:DeclareFunctions()
	return {
...
function modifier_Middle_infest_effect:Advanced_GetModifierBaseDamageOutgoing_Percentage()
	return self.bonus_attack
end

function modifier_Middle_infest_effect:AdvancedGetModifierHealthBonus()
	return self.bonus_health
end

function modifier_Middle_infest_effect:Advanced_GetModifierSpellAmplifyBonus()
	return self.bonus_spell
end
function modifier_Middle_infest_effect:Advanced_GetModifierTotalDamageOutgoing_Percentage()
	return self.outgoing
end
function modifier_Middle_infest_effect:Advanced_GetModifierPhysicalArmorBonus()
	return self.armor
end
function modifier_Middle_infest_effect:GetModifierMagicalResistanceBonus()
	return self.mrs
end


function modifier_Middle_infest_effect:OnTooltip()
