LinkLuaModifier("modifier_Middle_Thundergods_Wrath_Power", "skills/Middle_Thundergods_Wrath", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_hd_stormcrafter_active", "items/item_hd_stormcrafter", LUA_MODIFIER_MOTION_NONE)

Middle_Thundergods_Wrath = class({})

function Middle_Thundergods_Wrath:OnAbilityPhaseStart()
...
function Middle_Thundergods_Wrath:GetAOERadius()
	return 1500
end

function Middle_Thundergods_Wrath:OnAbilityPhaseInterrupted()
...
function Middle_Thundergods_Wrath:OnSpellStart() 
	if IsServer() then
		local ability 				= self
		...
		caster:AddNewModifier(caster, ability, "modifier_Middle_Thundergods_Wrath_Power", {duration = 20}):SetStackCount(i)
...
modifier_Middle_Thundergods_Wrath_Power = class({})

function modifier_Middle_Thundergods_Wrath_Power:IsDebuff()			return false end
function modifier_Middle_Thundergods_Wrath_Power:IsHidden() 		return false end
function modifier_Middle_Thundergods_Wrath_Power:IsPurgable() 		return false end
function modifier_Middle_Thundergods_Wrath_Power:IsPurgeException() return false end
function modifier_Middle_Thundergods_Wrath_Power:DeclareFunctions() return {MODIFIER_EVENT_ON_ATTACK_LANDED} end



function modifier_Middle_Thundergods_Wrath_Power:OnAttackLanded( params )
	...
	self.damage = ability:GetSpecialValueFor("basic_damage")*0.25 + ability:GetSpecialValueFor("bonus_damage")*caster:GetIntellect(false) *0.25
	...
	ParticleManager:SetParticleControl(particle, 1, Vector(pos.x, pos.y, pos.z+128))
	ParticleManager:SetParticleControl(particle, 2, Vector(pos.x, pos.y, pos.z))
	ParticleManager:ReleaseParticleIndex(particle)
