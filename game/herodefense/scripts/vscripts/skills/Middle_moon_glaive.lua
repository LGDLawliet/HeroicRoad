Middle_moon_glaive = class({})
LinkLuaModifier( "modifier_Middle_moon_glaive", "skills/Middle_moon_glaive", LUA_MODIFIER_MOTION_NONE )


function Middle_moon_glaive:GetIntrinsicModifierName()
	return "modifier_Middle_moon_glaive"
end
function Middle_moon_glaive:Precache( context )
	PrecacheResource( "particle", "particles/rebuild/spell/moon_glavive/moon_glaive_bounce.vpcf", context )
end
function Middle_moon_glaive:OnProjectileHit_ExtraData(target, location, keys)
	if not target then
		return
	end
...
function Middle_moon_glaive:GlaiveAttck(srouce,target, damage, bounce)
	local caster = self:GetCaster()
...
modifier_Middle_moon_glaive = advanced_modifier({})

function modifier_Middle_moon_glaive:IsHidden()	return true end
function modifier_Middle_moon_glaive:IsDebuff()	return false end
function modifier_Middle_moon_glaive:IsPurgable()	return false end
function modifier_Middle_moon_glaive:IsPurgeException() return false end
function modifier_Middle_moon_glaive:RemoveOnDeath() return false end
-- function Middle_moon_glaive:GetEffectName() return "particles/econ/items/bane/bane_fall20_immortal/bane_fall20_immortal_grip.vpcf" end

function modifier_Middle_moon_glaive:ADDeclareFunctions()
    return {
		MODIFIER_EVENT_ON_ATTACK_LANDED = {self:GetParent(),nil},
    }
end

function modifier_Middle_moon_glaive:OnAttackLanded(keys)
	if not IsServer() then return end
