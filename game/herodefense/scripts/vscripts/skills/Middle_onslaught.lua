Middle_onslaught = class({})

LinkLuaModifier( "modifier_Middle_onslaught_charge", "skills/Middle_onslaught", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Middle_onslaught", "skills/Middle_onslaught", LUA_MODIFIER_MOTION_NONE )

LinkLuaModifier( "modifier_Middle_onslaught_debuff", "skills/Middle_onslaught", LUA_MODIFIER_MOTION_NONE )
--------------------------------------------------------------------------------
-- Init Abilities
function Middle_onslaught:Precache( context )
...
function Middle_onslaught:OnSpellStart()
	-- unit identifier
	...
	local mod = caster:AddNewModifier(
		...
		self, -- ability source
		"modifier_Middle_onslaught_charge", -- modifier name
		{ duration = duration } -- kv
	)

end

function Middle_onslaught:OnChargeFinish( interrupt )
	-- unit identifier
	local caster = self:GetCaster()
	...
	local mod = caster:FindModifierByName( "modifier_Middle_onslaught_charge" )
	if mod then
	...
	caster:AddNewModifier(
		caster, -- player source
		self, -- ability source
		"modifier_Middle_onslaught", -- modifier name
		{ duration = duration } -- kv
	)
...
modifier_Middle_onslaught_charge = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_Middle_onslaught_charge:IsHidden()	return false end
function modifier_Middle_onslaught_charge:IsDebuff()	return false end
function modifier_Middle_onslaught_charge:IsPurgable()	return false end

--------------------------------------------------------------------------------
-- Initializations
function modifier_Middle_onslaught_charge:OnCreated( kv )
...
function modifier_Middle_onslaught_charge:OnRemoved()
	if not IsServer() then return end
...
function modifier_Middle_onslaught_charge:DeclareFunctions()
	local funcs = {
...
function modifier_Middle_onslaught_charge:GetOverrideAnimation()
	if self:GetParent():GetUnitName()=="npc_dota_hero_primal_beast" then
		return ACT_DOTA_IDLE
	end
	return ACT_DOTA_GENERIC_CHANNEL_1
end
function modifier_Middle_onslaught_charge:GetActivityTranslationModifiers()
	return "onslaught_windup"
end
function modifier_Middle_onslaught_charge:OnOrder( params )
...
function modifier_Middle_onslaught_charge:SetDirection( location )
	local dir = ((location-self.parent:GetOrigin())*Vector(1,1,0)):Normalized()
	self.target_angle = VectorToAngles( dir ).y
	self.face_target = false
end

function modifier_Middle_onslaught_charge:GetModifierMoveSpeed_Limit()
	return 0.1
end

--------------------------------------------------------------------------------
-- Status Effects
function modifier_Middle_onslaught_charge:CheckState()
	local state = {
...
function modifier_Middle_onslaught_charge:OrderFilter( data )
...
--------------------------------------------------------------------------------
-- Interval Effects
function modifier_Middle_onslaught_charge:OnIntervalThink()
	-- cancel logic
...
function modifier_Middle_onslaught_charge:TurnLogic( dt )
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_Middle_onslaught_charge:PlayEffects1()
	-- Get Resources
...
function modifier_Middle_onslaught_charge:SetEffects()
...
function modifier_Middle_onslaught_charge:PlayEffects2()
	-- Get Resources
...
modifier_Middle_onslaught = advanced_modifier({})


function modifier_Middle_onslaught:IsHidden()	return true end
function modifier_Middle_onslaught:IsDebuff()	return false end
function modifier_Middle_onslaught:IsPurgable()	return false end
function modifier_Middle_onslaught:IsMotionController() return true end
function modifier_Middle_onslaught:GetMotionControllerPriority() return DOTA_MOTION_CONTROLLER_PRIORITY_MEDIUM end

function modifier_Middle_onslaught:OnCreated( kv )
	self.parent = self:GetParent()
	self.ability = self:GetAbility()
	...
	if self.parent:HasAbility("heroTalent_npc_dota_hero_primal_beast_2") then
		self.turn_speed = self.turn_speed *3
		damage = damage + self.parent:GetAverageTrueAttackDamage(nil)
	end
	-- ability properties
...
function modifier_Middle_onslaught:OnDestroy()
	if not IsServer() then return end
...
function modifier_Middle_onslaught:DeclareFunctions()
	local funcs = {
		MODIFIER_EVENT_ON_ORDER,
...
function modifier_Middle_onslaught:OnOrder( params )
	if params.unit~=self:GetParent() then return end
...
function modifier_Middle_onslaught:GetModifierDisableTurning()
	return 1
end

function modifier_Middle_onslaught:SetDirection( location )
	local dir = ((location-self.parent:GetOrigin())*Vector(1,1,0)):Normalized()
	self.target_angle = VectorToAngles( dir ).y
	self.face_target = false
end

function modifier_Middle_onslaught:GetOverrideAnimation()
	return ACT_DOTA_RUN
end

function modifier_Middle_onslaught:GetActivityTranslationModifiers()
	return "onslaught_movement"
end



function modifier_Middle_onslaught:OnIntervalThink()   --对沿途敌人造成眩晕与窃取速度效果
...
function modifier_Middle_onslaught:TurnLogic( dt )
	-- only rotate when target changed
	if self.face_target then return end
...
function modifier_Middle_onslaught:HitLogic()
	-- destroy trees
	...
	for _,unit in pairs(units) do
		...
		if not self.knockback_units[unit] then
			...
			if is_enemy then
				...
				enemy:AddNewModifier(self.parent, self.ability, "modifier_Middle_onslaught_debuff",{ duration = 7 } )
...
function modifier_Middle_onslaught:GetEffectName()
	return "particles/rebuild/spell/onslaught/effect_charge_active.vpcf"
end

function modifier_Middle_onslaught:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_Middle_onslaught:PlayEffects( target, radius )
	-- Get Resources
...
function modifier_Middle_onslaught:CheckState()
	local state = {
...
function modifier_Middle_onslaught:ADDeclareFunctions()
    return 
    {
...
function modifier_Middle_onslaught:Advanced_GetModifier_FlyingPathing()	
	return 1
end
...
modifier_Middle_onslaught_debuff = advanced_modifier({})

function modifier_Middle_onslaught_debuff:IsDebuff() return true end
function modifier_Middle_onslaught_debuff:IsHidden() return false end
function modifier_Middle_onslaught_debuff:IsPurgable() return false end
function modifier_Middle_onslaught_debuff:IsPurgeException() return false end



function modifier_Middle_onslaught_debuff:DeclareFunctions()
	return { MODIFIER_PROPERTY_TOOLTIP}
end

function modifier_Middle_onslaught_debuff:OnTooltip()
    return self:Advanced_GetModifierPhysicalArmorBonus()
end

-- advanced_modifier

function modifier_Middle_onslaught_debuff:ADDeclareFunctions()
    return 
    {
        advanced_MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS,
    }
end
function modifier_Middle_onslaught_debuff:Advanced_GetModifierPhysicalArmorBonus()
    return -7
end