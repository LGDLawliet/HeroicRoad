...
function Timers:Think()
	...
	if GameRules:State_Get() > 10 --[[DOTA_GAMERULES_STATE_POST_GAME]] then
		return
	end
...
function Timers:ExecuteTimers(timerList, now)
	...
	if now >= currentTimer.endTime then
		...
		if currentTimer.context then
			...
			status, timerResult = xpcall(function() return currentTimer.callback(currentTimer) end, "")
		end
