...
Primary_Arcane_Replacement = class({})


LinkLuaModifier("modifier_Primary_Arcane_Replacement", "skills/Primary_Arcane_Replacement", LUA_MODIFIER_MOTION_NONE)
-- LinkLuaModifier("modifier_Primary_Arcane_Replacement_effect", "skills/Primary_Arcane_Replacement", LUA_MODIFIER_MOTION_NONE)


function Primary_Arcane_Replacement:GetIntrinsicModifierName() return "modifier_Primary_Arcane_Replacement" end
function Primary_Arcane_Replacement:IsHiddenWhenStolen() 		return false end
function Primary_Arcane_Replacement:IsRefreshable() 			return true  end
function Primary_Arcane_Replacement:Precache( context )
	PrecacheResource( "particle", "particles/rebuild/spell/arcane_replacement/effect.vpcf", context )
end


function Primary_Arcane_Replacement:ReleasePower(stack)
...
modifier_Primary_Arcane_Replacement= class({})

function modifier_Primary_Arcane_Replacement:IsDebuff()			return false end
function modifier_Primary_Arcane_Replacement:IsHidden() 			return false end
function modifier_Primary_Arcane_Replacement:IsPurgable() 		return false end
function modifier_Primary_Arcane_Replacement:IsPurgeException() 	return false end
function modifier_Primary_Arcane_Replacement:OnCreated(keys)
	if IsServer() then
...
function modifier_Primary_Arcane_Replacement:OnRefresh(keys)
	if IsServer() then
		local ability = self:GetAbility()
		self.change_percentage = ability:GetSpecialValueFor("change_percentage")*0.01
		-- self.change_limit = ability:GetSpecialValueFor("change_limit")
...
function modifier_Primary_Arcane_Replacement:OnCustomModifierFunction_Heal(keys)
	if IsServer() then
...
function modifier_Primary_Arcane_Replacement:OnIntervalThink(keys)
