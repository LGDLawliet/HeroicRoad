LinkLuaModifier("modifier_Primary_split_shot", "skills/Primary_split_shot", LUA_MODIFIER_MOTION_NONE)
Primary_split_shot = class({})

function Primary_split_shot:OnUpgrade()
...
function Primary_split_shot:OnOwnerSpawned()
	if self.toggle_state then
		self:ToggleAbility()
	end
end

function Primary_split_shot:OnOwnerDied()
	self.toggle_state = self:GetToggleState()
end

function Primary_split_shot:OnToggle()
	if not IsServer() then return end
	if self:GetToggleState() then
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_Primary_split_shot", {})
	else
		self:GetCaster():RemoveModifierByNameAndCaster("modifier_Primary_split_shot", self:GetCaster())
	end

end


modifier_Primary_split_shot	= advanced_modifier({})

function modifier_Primary_split_shot:IsHidden()	return false end
function modifier_Primary_split_shot:IsPurgable()	return false end
function modifier_Primary_split_shot:OnCreated(keys)
	if IsServer() then
...
function modifier_Primary_split_shot:OnRefresh(keys)
	if IsServer() then
		self.split_shot = self:GetAbility():GetSpecialValueFor("arrow_count")
		local talent3 = self:GetParent():FindAbilityByName("heroTalent_npc_dota_hero_medusa_3")
		if talent3 then
...
function modifier_Primary_split_shot:DeclareFunctions()
    return {
...
function modifier_Primary_split_shot:ADDeclareFunctions()
	return{
...
function modifier_Primary_split_shot:Advanced_GetModifierAttackSpeedPercentage()
	return -self:GetAbility():GetSpecialValueFor("attack_speed_down")
end

function modifier_Primary_split_shot:OnAttack(keys)
...
function modifier_Primary_split_shot:Advanced_GetModifierDamageOutgoing_Percentage()
	if not IsServer() then return end
