...
Primary_elder_dragon_form = class({})
LinkLuaModifier("modifier_Primary_elder_dragon_form_transform", "skills/Primary_elder_dragon_form", LUA_MODIFIER_MOTION_NONE)

function Primary_elder_dragon_form:Precache( context )
...
-- 指示器
function Primary_elder_dragon_form:GetIntrinsicModifierName()
	return "modifier_generic_custom_indicator"
end

function Primary_elder_dragon_form:CastFilterResultLocation( vLoc )
	if IsClient() then
...
function Primary_elder_dragon_form:CreateCustomIndicator()
	local particle_cast = "particles/ui_mouseactions/custom_sector_finder.vpcf"
	self.effect_cast = ParticleManager:CreateParticle( particle_cast, PATTACH_ABSORIGIN_FOLLOW, self:GetCaster() )
end

function Primary_elder_dragon_form:UpdateCustomIndicator( loc )
...
function Primary_elder_dragon_form:DestroyCustomIndicator()
	ParticleManager:DestroyParticle( self.effect_cast, true ) 
	ParticleManager:ReleaseParticleIndex( self.effect_cast )

end

function Primary_elder_dragon_form:GetCastRange()
	if IsServer() then
		return 30000
	end
	local caster = self:GetCaster()
...
-- 施法效果
function Primary_elder_dragon_form:OnSpellStart()
	local caster = self:GetCaster()
	local ability = self
	...
	caster:GameTimer(0.2,function ()
		...
		caster.Form_MODIFIER_NAME = "modifier_Primary_elder_dragon_form_transform"
		-- 变龙
		...
		caster:AddNewModifier(caster, ability, "modifier_Primary_elder_dragon_form_transform", {duration = duration})
	end)
end

function Primary_elder_dragon_form:Fireshoot(pos, damage_index, radius_index)
	if not IsServer() then return end
	local caster = self:GetCaster()
	-- 传参，伤害，长度，宽度，速度
...
function Primary_elder_dragon_form:OnProjectileHit_ExtraData( target, location,keys )
...
function Primary_elder_dragon_form:PlayEffect(target)
...
-------------------------------------------------------------------------------------------------------------------------------
modifier_Primary_elder_dragon_form_transform = advanced_modifier({})
function modifier_Primary_elder_dragon_form_transform:IsHidden()	return false end
function modifier_Primary_elder_dragon_form_transform:IsPurgable()	return false end
function modifier_Primary_elder_dragon_form_transform:IsDebuff()	return false end
function modifier_Primary_elder_dragon_form_transform:GetPriority() return MODIFIER_PRIORITY_ULTRA-1 end
function modifier_Primary_elder_dragon_form_transform:DeclareFunctions()	
	local decFuncs = {
...
-- 更改形态，更换弹道和攻击声音
function modifier_Primary_elder_dragon_form_transform:GetModifierModelScale() 
    return 40
end
function modifier_Primary_elder_dragon_form_transform:GetAttackSound()
	return "Hero_DragonKnight.ElderDragonShoot2.Attack"
end
function modifier_Primary_elder_dragon_form_transform:GetModifierModelChange()
	return "models/items/dragon_knight/fireborn_dragon/fireborn_dragon.vmdl"
end
function modifier_Primary_elder_dragon_form_transform:GetModifierProjectileName()
...
-- 改变攻击形态
function modifier_Primary_elder_dragon_form_transform:OnCreated()
...
-- 退出变身，属性复原
function modifier_Primary_elder_dragon_form_transform:OnDestroy()
    if IsServer() then    	
		local caster =self:GetCaster()
		--如果单位不是远程单位 则改变为远程
...
-- 攻击力，攻击距离，弹道速度
function modifier_Primary_elder_dragon_form_transform:ADDeclareFunctions()
    return 
    {
...
function modifier_Primary_elder_dragon_form_transform:Advanced_GetModifierPreAttack_BonusDamage() return self.bonus_attack end
function modifier_Primary_elder_dragon_form_transform:Advanced_GetModifierAttackRangeBonus() return  self.bonus_attack_range end
function modifier_Primary_elder_dragon_form_transform:GetModifierProjectileSpeedBonus() return 200 end
-- 溅射攻击
function modifier_Primary_elder_dragon_form_transform:OnAttackLanded(keys)
	if not IsServer() then return end
