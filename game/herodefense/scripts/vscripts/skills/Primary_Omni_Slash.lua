...
-- OMNI SLASH --
Primary_Omni_Slash = Primary_Omni_Slash or class({})
LinkLuaModifier("modifier_Primary_Omni_Slash_caster", "skills/Primary_Omni_Slash", LUA_MODIFIER_MOTION_NONE)



LinkLuaModifier("modifier_Primary_Omni_Slash_double", "skills/Primary_Omni_Slash", LUA_MODIFIER_MOTION_NONE)

function Primary_Omni_Slash:IsNetherWardStealable() return false end




function Primary_Omni_Slash:IsHiddenWhenStolen()return false end

-- Grimstroke edge case (really should be cleaner than this but...yeah)
function Primary_Omni_Slash:OnOwnerDied()
...
function Primary_Omni_Slash:OnOwnerSpawned()
	self:OnOwnerDied()
end

function Primary_Omni_Slash:OnUpgrade()
	if self:GetCaster():FindAbilityByName("Primary_Omni_Slash"):GetLevel() == 1 then
...
function Primary_Omni_Slash:OnAbilityPhaseStart()
...
function Primary_Omni_Slash:OnSpellStart()
    ...
    omnislash_modifier_handler = self.caster:AddNewModifier(self.caster, self, "modifier_Primary_Omni_Slash_caster", {duration = self:GetSpecialValueFor("duration")*ModifierStatusGain})
...
function Primary_Omni_Slash:TalentEffect(target,duration)
	local caster = self:GetCaster()
	local unit = caster:CreateDouble(target:GetOrigin(),caster:GetForwardVector())
	unit:AddNewModifier(caster, self, "modifier_Primary_Omni_Slash_double", {duration=duration})

end


modifier_Primary_Omni_Slash_caster = modifier_Primary_Omni_Slash_caster or class({})

function modifier_Primary_Omni_Slash_caster:OnCreated()
	self.caster = self:GetCaster()
...
function modifier_Primary_Omni_Slash_caster:OnIntervalThink()
...
function modifier_Primary_Omni_Slash_caster:BounceAndSlaughter(first_slash)
...
function modifier_Primary_Omni_Slash_caster:DeclareFunctions()
	return {
...
function modifier_Primary_Omni_Slash_caster:CheckState()
...
function modifier_Primary_Omni_Slash_caster:GetModifierPreAttack_BonusDamage(kv)
	return self:GetAbility():GetSpecialValueFor("bonus_damage")
end

function modifier_Primary_Omni_Slash_caster:GetOverrideAnimation()
	return ACT_DOTA_OVERRIDE_ABILITY_4
end

function modifier_Primary_Omni_Slash_caster:OnDestroy()
	if IsServer() then
...
modifier_Primary_Omni_Slash_double = modifier_Primary_Omni_Slash_double or class({})

function modifier_Primary_Omni_Slash_double:OnCreated()
	self.caster = self:GetCaster()
	self.parent = self:GetParent()
	self.ability = self:GetAbility()
...
function modifier_Primary_Omni_Slash_double:OnIntervalThink()
	self:BounceAndSlaughter()
    --更新攻击速度
	local slash_rate = (self.caster:GetSecondsPerAttack(false) / self:GetAbility():GetSpecialValueFor("attack_rate_multiplier")) 
	self:StartIntervalThink(-1)
...
function modifier_Primary_Omni_Slash_double:BounceAndSlaughter(first_slash)
	local order = FIND_ANY_ORDER
...
function modifier_Primary_Omni_Slash_double:DeclareFunctions()
	return {
		MODIFIER_PROPERTY_PREATTACK_BONUS_DAMAGE,
		MODIFIER_PROPERTY_OVERRIDE_ANIMATION
	}
...
function modifier_Primary_Omni_Slash_double:CheckState()
	local state = {[MODIFIER_STATE_INVULNERABLE] = true,
				[MODIFIER_STATE_UNSELECTABLE] = true,
				[MODIFIER_STATE_IGNORING_MOVE_AND_ATTACK_ORDERS] = true,
				[MODIFIER_STATE_NOT_ON_MINIMAP] = true,
...
function modifier_Primary_Omni_Slash_double:GetOverrideAnimation()
	return ACT_DOTA_OVERRIDE_ABILITY_4
end

function modifier_Primary_Omni_Slash_double:OnDestroy()
	if IsServer() then
