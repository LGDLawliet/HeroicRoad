LinkLuaModifier("modifier_Primary_ravage_debuff", "skills/Primary_ravage", LUA_MODIFIER_MOTION_NONE)

Primary_ravage = Primary_ravage or class({})

function Primary_ravage:Precache(context)
...
function Primary_ravage:GetCastRange(location, target)
    return self:GetSpecialValueFor("radius") - self:GetCaster():GetCastRangeBonus()
end

function Primary_ravage:OnSpellStart()
...
function Primary_ravage:CastEffect(keys)
    local caster = self:GetCaster()
...
--damage
--duration
function Primary_ravage:ApplyEffect(keys)
    local caster = self:GetCaster()
    ...
    local stun_duration = duration * StatusResistance
    if stun_duration > 0 then
        target:AddNewModifier(caster, self, "modifier_Primary_ravage_debuff", {duration = stun_duration})
    end
...
--------------
modifier_Primary_ravage_debuff = modifier_Primary_ravage_debuff or advanced_modifier({})

function modifier_Primary_ravage_debuff:IsHidden() return false end
function modifier_Primary_ravage_debuff:IsDebuff() return true end
function modifier_Primary_ravage_debuff:IsPurgable() return false end
function modifier_Primary_ravage_debuff:IsPurgeException() return true end

function modifier_Primary_ravage_debuff:OnCreated()
    local caster = self:GetCaster()
    local parent = self:GetParent()
    local ability = self:GetAbility()
end

function modifier_Primary_ravage_debuff:OnRefresh()
    self:OnCreated()
end

function modifier_Primary_ravage_debuff:CheckState()
    return {
        [MODIFIER_STATE_STUNNED] = true,
    }
end

function modifier_Primary_ravage_debuff:OnDestroy()
    local caster = self:GetCaster()
    local parent = self:GetParent()
    if IsServer() then
...
function modifier_Primary_ravage_debuff:DeclareFunctions()
    return {
        MODIFIER_PROPERTY_TOOLTIP
    }
end

function modifier_Primary_ravage_debuff:OnTooltip()
