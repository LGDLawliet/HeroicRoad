Primary_Thunderstrike = class({})
LinkLuaModifier("modifier_Primary_Thunderstrike_effect", "skills/Primary_Thunderstrike", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_Thunderstrike_ready", "skills/Primary_Thunderstrike", LUA_MODIFIER_MOTION_NONE)

--Abilities
function Primary_Thunderstrike:GetIntrinsicModifierName() return "modifier_Primary_Thunderstrike_effect" end
function Primary_Thunderstrike:GetAOERadius()
	return self:GetSpecialValueFor("radius")
end
function Primary_Thunderstrike:Precache( context )
...
modifier_Primary_Thunderstrike_effect = class({})
function modifier_Primary_Thunderstrike_effect:IsHidden() return true end
function modifier_Primary_Thunderstrike_effect:IsDebuff() return false end
function modifier_Primary_Thunderstrike_effect:IsPurgable() return false end
function modifier_Primary_Thunderstrike_effect:IsPurgeException() return false end
function modifier_Primary_Thunderstrike_effect:IsStunDebuff() return false end
function modifier_Primary_Thunderstrike_effect:AllowIllusionDuplicate() return false end

function modifier_Primary_Thunderstrike_effect:OnCreated()
...
function modifier_Primary_Thunderstrike_effect:OnRefresh()
    if not IsServer() then
        return
    end
    self:StartIntervalThink(0.5)
...
function modifier_Primary_Thunderstrike_effect:OnIntervalThink()
    if not IsServer() then
        return
    end
	if self:GetAbility():IsCooldownReady() then
		self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_Primary_Thunderstrike_ready", {})
	end
    
end

modifier_Primary_Thunderstrike_ready = class({})
function modifier_Primary_Thunderstrike_ready:IsHidden() return true end
function modifier_Primary_Thunderstrike_ready:IsDebuff() return false end
function modifier_Primary_Thunderstrike_ready:IsPurgable() return false end
function modifier_Primary_Thunderstrike_ready:IsPurgeException() return false end
function modifier_Primary_Thunderstrike_ready:IsStunDebuff() return false end
function modifier_Primary_Thunderstrike_ready:AllowIllusionDuplicate() return false end
function modifier_Primary_Thunderstrike_ready:RemoveOnDeath() return false end
function modifier_Primary_Thunderstrike_ready:OnCreated()
    if not IsServer() then
        return
    end
...
function modifier_Primary_Thunderstrike_ready:OnIntervalThink()
    if not IsServer() then
        return
    end
