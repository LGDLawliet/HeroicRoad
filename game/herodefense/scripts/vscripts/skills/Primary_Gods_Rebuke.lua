...
--------------------------------------------------------------------------------
Primary_Gods_Rebuke = class({})
LinkLuaModifier( "modifier_Primary_Gods_Rebuke", "skills/Primary_Gods_Rebuke", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_imgeneric_knockback_lua", "modifier/modifier_imgeneric_knockback_lua", LUA_MODIFIER_MOTION_BOTH )




function Primary_Gods_Rebuke:IsHiddenWhenStolen()return false end
function Primary_Gods_Rebuke:IsStealable()return true end
function Primary_Gods_Rebuke:IsNetherWardStealable()return false end
function Primary_Gods_Rebuke:IsRefreshable()return true end
function Primary_Gods_Rebuke:GetCastRange()
	if IsServer() then
		return 30000
	end
	if IsClient() then
...
function Primary_Gods_Rebuke:Precache( context )
...
--------------------------------------------------------------------------------
--指示器参考上界重锤
function Primary_Gods_Rebuke:GetIntrinsicModifierName()
	return "modifier_generic_custom_indicator"
end


function Primary_Gods_Rebuke:CastFilterResultLocation( vLoc )
	if IsClient() then
...
function Primary_Gods_Rebuke:CreateCustomIndicator()
...
function Primary_Gods_Rebuke:UpdateCustomIndicator( loc )
	-- get data
...
function Primary_Gods_Rebuke:DestroyCustomIndicator()
	-- print("xiao hui")
...
function Primary_Gods_Rebuke:OnSpellStart()
	-- unit identifier
	...
	local buff = caster:AddNewModifier(
		...
		self, -- ability source
		"modifier_Primary_Gods_Rebuke", -- modifier name
		{  } -- kv
	)
...
--------------------------------------------------------------------------------
-- Play Effects
function Primary_Gods_Rebuke:PlayEffects1( caught, direction )
	-- Get Resources
...
function Primary_Gods_Rebuke:PlayEffects2( target, origin, direction )
	-- Get Resources
...
function Primary_Gods_Rebuke:TalentEffect(pos)
...
modifier_Primary_Gods_Rebuke = advanced_modifier({})

function modifier_Primary_Gods_Rebuke:IsHidden()return true end
function modifier_Primary_Gods_Rebuke:IsDebuff()return false end
function modifier_Primary_Gods_Rebuke:IsPurgable()return false end
function modifier_Primary_Gods_Rebuke:OnCreated( kv )
	self.bonus_crit = self:GetAbility():GetSpecialValueFor( "bonus_damage" )
end
function modifier_Primary_Gods_Rebuke:Advanced_GetModifierCriticalStrike( keys )
	return self.bonus_crit
end
function modifier_Primary_Gods_Rebuke:ADDeclareFunctions()
    return 
    {
