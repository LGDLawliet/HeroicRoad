...
Middle_aphotic_shield = class({})

LinkLuaModifier("modifier_Middle_aphotic_shield", "skills/Middle_aphotic_shield", LUA_MODIFIER_MOTION_NONE)

function Middle_aphotic_shield:IsHiddenWhenStolen() 		return false end
function Middle_aphotic_shield:IsRefreshable() 			return true end
function Middle_aphotic_shield:IsStealable() 				return true end
function Middle_aphotic_shield:IsNetherWardStealable()	return true end


function Middle_aphotic_shield:OnSpellStart()
	local target = self:GetCursorTarget()
	local buff = target:FindModifierByName("modifier_Middle_aphotic_shield")
	if buff then
		buff:SetStackCount(0)
		buff:SafeDestroy()
	end
	local ModifierStatusGain = self:GetCaster():GetModifierDurationGainIndex(1)
	target:AddNewModifier(self:GetCaster(), self, "modifier_Middle_aphotic_shield", {duration = self:GetSpecialValueFor("duration")*ModifierStatusGain})
	target:Purge(false, true, false, true, false)
end

modifier_Middle_aphotic_shield = advanced_modifier({})

function modifier_Middle_aphotic_shield:IsDebuff()			return false end
function modifier_Middle_aphotic_shield:IsHidden() 			return false end
function modifier_Middle_aphotic_shield:IsPurgable() 			return true end
function modifier_Middle_aphotic_shield:IsPurgeException() 	return true end

function modifier_Middle_aphotic_shield:OnCreated()
	if IsServer() then
...
function modifier_Middle_aphotic_shield:OnDestroy()
...
function modifier_Middle_aphotic_shield:ADDeclareFunctions()
	return {
...
function modifier_Middle_aphotic_shield:AdvancedGetModifierTotal_ConstantBlock(keys)
	...
	--过滤低于30的伤害 
	if keys.damage<=30  then
		return keys.damage
	end
	--15%%几率抵消所有伤害
	if 15>=RandomInt(1, 100) and keys.damage<self:GetParent():GetMaxHealth()*0.5 then
		return keys.damage
	end
