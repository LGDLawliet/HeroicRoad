Primary_Chaos_Meteor = class({})
LinkLuaModifier( "modifier_Primary_Chaos_Meteor_thinker", "skills/Primary_Chaos_Meteor", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Primary_Chaos_Meteor_debuff", "skills/Primary_Chaos_Meteor", LUA_MODIFIER_MOTION_NONE )


function Primary_Chaos_Meteor:OnSpellStart()
	-- unit identifier
	...
	CreateModifierThinker(
		...
		self, -- ability source
		"modifier_Primary_Chaos_Meteor_thinker", -- modifier name
		{}, -- kv
		point,
...
modifier_Primary_Chaos_Meteor_thinker = class({})

function modifier_Primary_Chaos_Meteor_thinker:IsHidden()	return true end


function modifier_Primary_Chaos_Meteor_thinker:OnCreated( kv )
	if IsServer() then
		-- references
...
function modifier_Primary_Chaos_Meteor_thinker:OnDestroy( kv )
	if IsServer() then
		-- add vision
...
--------------------------------------------------------------------------------
-- Interval Effects
function modifier_Primary_Chaos_Meteor_thinker:OnIntervalThink()
...
function modifier_Primary_Chaos_Meteor_thinker:Burn()
	...
	for _,enemy in pairs(enemies) do
		if not self.effect_unit[enemy] then
			...
			enemy:AddNewModifier(caster, self:GetAbility(), "modifier_Primary_Chaos_Meteor_debuff", {duration = self:GetAbility():GetSpecialValueFor("duration")*StatusResistance})
		end
...
--------------------------------------------------------------------------------
-- Motion effects
function modifier_Primary_Chaos_Meteor_thinker:Move_Burn()
	if not self:GetCaster() then
		return
	end
...
function modifier_Primary_Chaos_Meteor_thinker:PlayEffects1()
	if not self:GetCaster() then
		return
	end
	-- Get Resources
...
function modifier_Primary_Chaos_Meteor_thinker:PlayEffects2()
	if not self:GetCaster() then
		return
	end
...
modifier_Primary_Chaos_Meteor_debuff = class({})

function modifier_Primary_Chaos_Meteor_debuff:IsDebuff()			return true end
function modifier_Primary_Chaos_Meteor_debuff:IsHidden() 			return false end
function modifier_Primary_Chaos_Meteor_debuff:IsPurgable() 			return true end
function modifier_Primary_Chaos_Meteor_debuff:IsPurgeException() 	return true end
function modifier_Primary_Chaos_Meteor_debuff:GetEffectName()
	return "particles/units/heroes/hero_invoker/invoker_chaos_meteor_burn_debuff.vpcf"
end

function modifier_Primary_Chaos_Meteor_debuff:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end


function modifier_Primary_Chaos_Meteor_debuff:OnCreated()
	if IsServer() then
...
function modifier_Primary_Chaos_Meteor_debuff:OnIntervalThink()
	if not self:GetAbility() then
		self:SafeDestroy()
		return
	end
