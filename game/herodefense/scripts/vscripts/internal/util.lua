...
function TrueKill(caster, target, ability)
	...
	if target:IsAlive() then
			...
			target:Kill(ability, caster)
			target:ModifyHealth(0,ability,false, 0)
			--
	...
	for i=1, 4 do
		for j=1, #CDOTAGamerules.IMBA_TOWER[nTeamnumber][i] do
			...
			if tower and not tower:IsNull() and tower:IsAlive() then
				...
				local upgrade = false
				for k=0, tower:GetAbilityCount() - 1 do
...
function CDOTA_Modifier_Lua:CheckMotionControllers()
	...
	local modifier_priority = 0
	if self.GetMotionControllerPriority then
		modifier_priority = self:GetMotionControllerPriority()
	end
	
	local is_motion_controller = false
	local motion_controller_priority = 0
		...
		if self ~= modifier then			
			...
			if modifier.IsMotionController then
				if modifier:IsMotionController() then
					...
					if modifier.GetMotionControllerPriority then
	...
	if is_motion_controller and motion_controller_priority then
		...
		elseif motion_controller_priority == modifier_priority then			
			if found_modifier_handler:GetCreationTime() >= self:GetCreationTime() then				
				...
				found_modifier_handler:SafeDestroy()
				return true
			end
...
function BitWiseAbilityBehavior(hAbility,pszAbilityBehavior)
	...
	--print("bit result",hAbility:GetName(),pszAbilityBehavior)
	return false
end
...
function DoHDCleaveAttack(hAttacker, hTarget, hAbility, fDamage, fStartRadius, fEndRadius, fDistance, sHitEffect)
	...
	if sHitEffect then
		...
		local pos =  hAttacker:IsRangedAttacker() and hTarget:GetAbsOrigin() or hAttacker:GetAbsOrigin()
		pos.z = pos.z +64
		ParticleManager:SetParticleControl(pfx, 0,pos)
		ParticleManager:SetParticleControl(pfx, 1,Vector(0,0,1))
		ParticleManager:SetParticleControl(pfx, 2,pos)
		-- ParticleManager:SetParticleControl(pfx, 3,pos+hAttacker:GetForwardVector()*100)
		-- ParticleManager:SetParticleControl(pfx, 4,pos+hAttacker:GetForwardVector()*100)
		-- ParticleManager:SetParticleControl(pfx, 5,pos+hAttacker:GetForwardVector()*100)
		-- ParticleManager:SetParticleControl(pfx, 6,pos+hAttacker:GetForwardVector()*100)
		-- ParticleManager:SetParticleControl(pfx, 7,pos+hAttacker:GetForwardVector()*100)
		-- ParticleManager:SetParticleControl(pfx, 8,pos+hAttacker:GetForwardVector()*100)
		-- ParticleManager:SetParticleControl(pfx, 9,pos+hAttacker:GetForwardVector()*100)
		-- ParticleManager:SetParticleControl(pfx, 10,pos+hAttacker:GetForwardVector()*100)
	...
	for i=1, #enemy do
		if enemy[i] ~= hTarget then
			ApplyDamage({attacker = hAttacker, victim = enemy[i], ability = hAbility, damage = fDamage, damage_type = DAMAGE_TYPE_PHYSICAL, damage_flags = DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL })
			if pfx then
