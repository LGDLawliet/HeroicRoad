modifier_diff_3 = advanced_modifier({})

function modifier_diff_3:IsHidden()return false end
function modifier_diff_3:IsDebuff()return false end
function modifier_diff_3:IsPurgable()return false end
function modifier_diff_3:IsPurgeException() 	return false end
function modifier_diff_3:RemoveOnDeath() return true end
function modifier_diff_3:GetAttributes() return MODIFIER_ATTRIBUTE_IGNORE_INVULNERABLE end
function modifier_diff_3:GetTexture() return self.texture end

function modifier_diff_3:OnCreated(keys)
    if IsServer() then
...
function modifier_diff_3:ModifyTaskData(data)
...
--每过回合征召怪物cy
function modifier_diff_3:ADDeclareFunctions()
    return 
    {
...
function modifier_diff_3:OnChaoticEraRoundChange(keys)
    local round = keys.round
    local round_index = 0.2 + round/36
    if round >= 24 then
        round_index = round_index + round/80
        if round >= 30 then
            round_index = round_index + round/115
        end
    end
...
function modifier_diff_3:AdvancedModifyChaoticEraSpwnData(attribute,unit)
    if unit then
