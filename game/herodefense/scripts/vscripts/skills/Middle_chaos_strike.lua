...
Middle_chaos_strike = class({})
LinkLuaModifier( "modifier_Middle_chaos_strike", "skills/Middle_chaos_strike", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------
-- Passive Modifier
function Middle_chaos_strike:GetIntrinsicModifierName()
	return "modifier_Middle_chaos_strike"
end



function Middle_chaos_strike:Precache( context )
...
modifier_Middle_chaos_strike = advanced_modifier({})


function modifier_Middle_chaos_strike:IsHidden()	return true end
function modifier_Middle_chaos_strike:IsPurgable()	return false end
function modifier_Middle_chaos_strike:IsPurgeException() return false end
--------------------------------------------------------------------------------
-- Initializations
function modifier_Middle_chaos_strike:OnCreated( kv )
	-- references
...
function modifier_Middle_chaos_strike:OnRefresh( kv )
	-- references
	self.crit_chance = self:GetAbility():GetSpecialValueFor( "crit_chance" )
	self.crit_damage_min = self:GetAbility():GetSpecialValueFor( "crit_damage_min" )
	self.crit_damage_max = self:GetAbility():GetSpecialValueFor( "crit_damage_max" )
...
function modifier_Middle_chaos_strike:DeclareFunctions()
	local funcs = {
...
function modifier_Middle_chaos_strike:Advanced_GetModifierCriticalStrike( keys )
	if IsServer() and (not self:GetParent():PassivesDisabled()) then
		--混沌系统
		local chance = self.crit_chance + RandomInt(-5, 15)
		local ability = self:GetParent():FindAbilityByName("heroTalent_npc_dota_hero_chaos_knight_4")
		if ability then
			chance = chance +ability:GetBonusChance()
...
function modifier_Middle_chaos_strike:OnTakeDamage( params )
	if IsServer() then
		-- filter
		local pass = false
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_Middle_chaos_strike:PlayEffects( target )
	-- get resource
...
function modifier_Middle_chaos_strike:ADDeclareFunctions()
    return 
    {
