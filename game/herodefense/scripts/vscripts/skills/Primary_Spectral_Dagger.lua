...
LinkLuaModifier("modifier_Primary_Spectral_Dagger_debuff", "skills/Primary_Spectral_Dagger", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_Spectral_Dagger_ghost", "skills/Primary_Spectral_Dagger", LUA_MODIFIER_MOTION_NONE)
...
function Primary_Spectral_Dagger:GetCastRange()
...
function Primary_Spectral_Dagger:OnSpellStart()
	...
	-- hCaster:AddNewModifier(hCaster, self, "modifier_Primary_Spectral_Dagger_buff", { duration = duration })
...
function Primary_Spectral_Dagger:Precache( context )
	PrecacheResource( "particle", "particles/econ/items/spectre/spectre_arcana/spectre_arcana_desolate.vpcf", context )

end

function Primary_Spectral_Dagger:GetIntrinsicModifierName()
	return "modifier_Primary_Spectral_Dagger"
end

---------------------------------------------------------------------
--Modifiers
modifier_Primary_Spectral_Dagger = modifier_Primary_Spectral_Dagger or class({})
function modifier_Primary_Spectral_Dagger:IsHidden()	return true end
function modifier_Primary_Spectral_Dagger:IsDebuff()	return false end
...
function modifier_Primary_Spectral_Dagger:OnCreated(params)
	if IsServer() then
		...
		self.tGhosts = {}
		-- self.unique_str = DoUniqueString("modifier_Primary_Spectral_Dagger_buff")
...
function modifier_Primary_Spectral_Dagger:OnRefresh(keys)
	if IsServer() then
		local hAbility = self:GetAbility()
		self.radius = hAbility:GetSpecialValueFor("move_radius")
		self.spirits = hAbility:GetSpecialValueFor("dagger_count")
...
function modifier_Primary_Spectral_Dagger:OnDestroy()
	if IsServer() then
		local hParent = self:GetParent()
		...
		for n, hGhost in pairs(self.tGhosts) do
			if IsValid(hGhost.hUnit) then
				hGhost.hUnit:RemoveModifierByName("modifier_Primary_Spectral_Dagger_ghost")
			end
		end
	end
end
function modifier_Primary_Spectral_Dagger:OnIntervalThink()
	if IsServer() then
		...
		if #(self.tGhosts) < maxCount then
			...
			local hGhost = {
				hUnit = CreateModifierThinker(hCaster, hAbility, "modifier_Primary_Spectral_Dagger_ghost", nil, vPosition, hCaster:GetTeamNumber(), false),
				vTargetPosition = nil,
				hTarget = nil,
				bReturning = false
			}
...
function modifier_Primary_Spectral_Dagger:GetMaxDaggerCount()
	return self.spirits
end
...
---------------------------------------------------------------------
modifier_Primary_Spectral_Dagger_ghost = modifier_Primary_Spectral_Dagger_ghost or advanced_modifier({})
function modifier_Primary_Spectral_Dagger_ghost:IsHidden()	return true end
function modifier_Primary_Spectral_Dagger_ghost:IsDebuff()	return false end
function modifier_Primary_Spectral_Dagger_ghost:IsPurgable()	return false end
function modifier_Primary_Spectral_Dagger_ghost:IsPurgeException()	return false end
function modifier_Primary_Spectral_Dagger_ghost:IsStunDebuff()	return false end
function modifier_Primary_Spectral_Dagger_ghost:AllowIllusionDuplicate()	return false end
function modifier_Primary_Spectral_Dagger_ghost:OnCreated(params)
	if IsServer() then
...
function modifier_Primary_Spectral_Dagger_ghost:OnDestroy()
	if IsServer() then
		self:GetParent():ForceKill(false)
	end
end
function modifier_Primary_Spectral_Dagger_ghost:CheckState()
	return {
...
function modifier_Primary_Spectral_Dagger_ghost:OnIntervalThink()
	local parent = self:GetParent()
		...
		for _, unit in ipairs(tTargets) do
			...
			if self.damage_list[unit]<time then
				...
				if unit:IsAlive() then
					...
					unit:AddNewModifier(caster, ability, "modifier_Primary_Spectral_Dagger_debuff", { duration = debuff_duration  * StatusResistance})
				end
			end
		end
...
function modifier_Primary_Spectral_Dagger_ghost:ADDeclareFunctions()
	return {
		MODIFIER_EVENT_ON_Wave_End = {},
	}
end
function modifier_Primary_Spectral_Dagger_ghost:OnWaveEnd()
    if IsClient() then
        return
    end
...
if modifier_Primary_Spectral_Dagger_debuff == nil then
	modifier_Primary_Spectral_Dagger_debuff = class({})
end
function modifier_Primary_Spectral_Dagger_debuff:IsHidden()return false end
function modifier_Primary_Spectral_Dagger_debuff:IsDebuff()return self:GetParent():GetTeamNumber() ~= self:GetCaster():GetTeamNumber() end
function modifier_Primary_Spectral_Dagger_debuff:IsPurgable()return false end
function modifier_Primary_Spectral_Dagger_debuff:IsPurgeException()return false end
function modifier_Primary_Spectral_Dagger_debuff:IsStunDebuff()return false end
function modifier_Primary_Spectral_Dagger_debuff:AllowIllusionDuplicate()return false end
function modifier_Primary_Spectral_Dagger_debuff:OnCreated(table)
	self.value = self:GetAbility():GetSpecialValueFor("bonus_movespeed") 
end
function modifier_Primary_Spectral_Dagger_debuff:DeclareFunctions()
	return {
		MODIFIER_PROPERTY_MOVESPEED_BONUS_CONSTANT,

	}
end
function modifier_Primary_Spectral_Dagger_debuff:GetModifierMoveSpeedBonus_Constant(params)
