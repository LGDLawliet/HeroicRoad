...
function frozen_giant_dance_smash:OnSpellStart()
	...
	self:GetCaster():AddNewModifier( self:GetCaster(), self, "modifier_absolute_no_cc", { duration =5.5 } )
	self.stomp_radius = self:GetSpecialValueFor( "stomp_radius" )
	self.stomp_damage = self:GetSpecialValueFor( "stomp_damage" )*self:GetCaster():GetDamageMax()
