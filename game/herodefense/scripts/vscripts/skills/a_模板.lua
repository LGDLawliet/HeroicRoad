...
-- LinkLuaModifier("modifier_Primary_reactive_armor_arua", "skills/Primary_reactive_armor", LUA_MODIFIER_MOTION_NONE)
-- LinkLuaModifier("modifier_Primary_reactive_armor_arua_effect", "skills/Primary_reactive_armor", LUA_MODIFIER_MOTION_NONE)
...
-- LinkLuaModifier("modifier_Primary_reactive_armor_effect", "skills/Primary_reactive_armor", LUA_MODIFIER_MOTION_NONE)
-- LinkLuaModifier("modifier_Primary_reactive_armor_effect2", "skills/Primary_reactive_armor", LUA_MODIFIER_MOTION_NONE)
-- LinkLuaModifier("modifier_Primary_reactive_armor_active_standby", "skills/Primary_reactive_armor", LUA_MODIFIER_MOTION_NONE)
-- LinkLuaModifier("modifier_Primary_reactive_armor_debuff", "skills/Primary_reactive_armor", LUA_MODIFIER_MOTION_NONE)
-- LinkLuaModifier("modifier_Primary_reactive_armor_thinker", "skills/Primary_reactive_armor", LUA_MODIFIER_MOTION_NONE)
...
-- self:GetCaster():GetRandomEffect(5,INT_TYPE,1)  > RandomInt(1, 100)
...
-- function Primary_reactive_armor:CastFilterResult()
-- 	-- check nohammer
-- 	if IsClient() then
-- 		return
-- 	end
...
-- function Primary_reactive_armor:GetCustomCastError()
-- 	if IsClient() then
-- 		return
-- 	end
-- 	local time = GameRules:GetTimeOfDay()
...
-- function modifier_Primary_reactive_armor:OnIntervalThink()
-- 	if IsServer() then
...
-- 			   local buffs = unit:FindAllModifiersByName("modifier_Primary_reactive_armor_effect")
-- 			   if  #buffs == 0 then
-- 				self:GetAbility():StartCooldown(7)
-- 				local ModifierStatusGain = self:GetParent():GetModifierDurationGainIndex(1)
-- 				unit:AddNewModifier(unit, self:GetAbility(), "modifier_Primary_reactive_armor_effect", {duration = self:GetAbility():GetSpecialValueFor("duration")*ModifierStatusGain})
-- 				return
-- 			   end
-- 		   end
-- 		   local units = FindUnitsInRadius(self:GetParent():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil,  1000,
...
function modifier_Primary_reactive_armor:DeclareFunctions()
	return {
		...
		-- MODIFIER_PROPERTY_PHYSICAL_CONSTANT_BLOCK,          --物理伤害阻挡
		...
		-- MODIFIER_PROPERTY_MAGICAL_CONSTANT_BLOCK,           --魔法伤害抵挡
...
-- 			-- print("take damage")
-- 			if keys.damage >= keys.unit:GetHealth() and 100>=RandomInt(1, 100)   then
-- 				-- keys.target:Heal(500, self:GetParent())
-- 				self:GetAbility():StartCooldown(5)
-- 				keys.unit:SetHealth(1+keys.damage)
-- 				keys.unit:Heal(500, self:GetParent())
-- 			end
-- 			-- local caster = self:GetCaster()
-- 			-- local gain = caster:GetModifierDurationGainIndex(1)
-- 			-- self:GetParent():AddNewModifier(caster, self:GetAbility(), "modifier_Primary_reactive_armor_active_effect", {duration = 3*gain})
-- 		end
...
-- function modifier_Primary_reactive_armor:OnDamageCalculated(keys)
...
-- 				-- keys.target:Heal(500, self:GetParent())
-- 				keys.target:SetHealth(5+keys.damage)
...
-- LinkLuaModifier("modifier_Advanced_Bad_Juju_specialValue", "skills/Advanced_Bad_Juju", LUA_MODIFIER_MOTION_NONE)

-- function Advanced_Bad_Juju:GetIntrinsicModifierName() return "modifier_Advanced_Bad_Juju_specialValue" end

-- modifier_Advanced_Bad_Juju_specialValue = class({})

-- function modifier_Advanced_Bad_Juju_specialValue:IsDebuff()			return false end
-- function modifier_Advanced_Bad_Juju_specialValue:IsHidden() 			return true end
-- function modifier_Advanced_Bad_Juju_specialValue:IsPurgable() 		return false end
-- function modifier_Advanced_Bad_Juju_specialValue:IsPurgeException() 	return false end

-- function modifier_Advanced_Bad_Juju_specialValue:DeclareFunctions() 
--     return {
...
-- function modifier_Advanced_Bad_Juju_specialValue:GetModifierOverrideAbilitySpecial() return 1 end

-- function modifier_Advanced_Bad_Juju_specialValue:GetModifierOverrideAbilitySpecialValue( params )
