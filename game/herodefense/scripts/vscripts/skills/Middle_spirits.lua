...
Middle_spirits = class({})
LinkLuaModifier("modifier_Middle_spirits", "skills/Middle_spirits", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Middle_spirits_handler", "skills/Middle_spirits", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Middle_spirits_creep_hit", "skills/Middle_spirits", LUA_MODIFIER_MOTION_NONE)


require('internal/timers')   --计时器功能
function Middle_spirits:OnSpellStart()
	if IsServer() then
		self.caster 					= self:GetCaster()
		self.ability 					= self.caster:FindAbilityByName("Middle_spirits")
		...
		local explosion_damage			= 2*self.caster:GetIntellect(false)
		...
		if self.caster:HasModifier("modifier_Middle_spirits") then
			self.caster:RemoveModifierByName("modifier_Middle_spirits")
		end
...
function Middle_spirits:Explode(caster, spirit, explosion_radius, explosion_damage, ability)
	if IsServer() then
...
--		SPIRITS	modifier	--
------------------------------
modifier_Middle_spirits = class({})
function modifier_Middle_spirits:IsPurgable() return false end
function modifier_Middle_spirits:IsPurgeException() return false end

function modifier_Middle_spirits:OnCreated(params)
	if IsServer() then
...
function modifier_Middle_spirits:OnIntervalThink()
	if IsServer() then
		...
		if ability.spirits_num_spirits < idealNumSpiritsSpawned then
			...
			newSpirit:AddNewModifier(
				caster, 
				ability, 
				"modifier_Middle_spirits_handler", 
				{ 
					duraiton 			= -1,
		...
		if ability.spirits_num_spirits == self.max_spirits and numSpiritsAlive == 0 then
			...
			caster:RemoveModifierByName("modifier_Middle_spirits")
			return
		end
	end
end
...
function modifier_Middle_spirits:OnRemoved()
	if IsServer() then
		...
		for k,spirit in pairs( ability.spirits_spiritsSpawned ) do
			if not spirit:IsNull() then
				spirit:RemoveModifierByName("modifier_Middle_spirits_handler")
			end
		end
...
--命中特效
modifier_Middle_spirits_creep_hit = class({})
function modifier_Middle_spirits_creep_hit:IsHidden() return true end
function modifier_Middle_spirits_creep_hit:OnCreated() 
	if IsServer() then
...
function modifier_Middle_spirits_creep_hit:OnRemoved()
	if IsServer() then
...
----------------------------------------------------------------------
modifier_Middle_spirits_handler = class({})
function modifier_Middle_spirits_handler:CheckState()
	local state = {
...
function modifier_Middle_spirits_handler:OnCreated(params)
	if IsServer() then
		self.caster 			= self:GetCaster()
...
function modifier_Middle_spirits_handler:OnIntervalThink()
	if IsServer() then 
...
function modifier_Middle_spirits_handler:OnHit(caster, spirit, enemies_hit, creep_damage, ability) 
	...
	for _,enemy in pairs(enemies_hit) do
		...
		if enemy:IsAlive() and not spirit:IsNull() then 
			...
			if spirit.hit_table[enemy] == nil then
				...
				enemy:AddNewModifier(caster, ability, "modifier_Middle_spirits_creep_hit", {duration = 0.03})
...
function modifier_Middle_spirits_handler:OnRemoved()
	if IsServer() then
		local spirit	= self:GetParent()
		local ability	= self:GetAbility()
