...
Primary_Desolate = class({})

LinkLuaModifier("modifier_Primary_Desolate_passive", "skills/Primary_Desolate", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_Desolate_alone", "skills/Primary_Desolate", LUA_MODIFIER_MOTION_NONE)

function Primary_Desolate:GetIntrinsicModifierName()
	return "modifier_Primary_Desolate_passive"
end
----------------------------------------------
modifier_Primary_Desolate_passive = advanced_modifier({})

function modifier_Primary_Desolate_passive:IsDebuff()			return false end
function modifier_Primary_Desolate_passive:IsHidden() 			return true end
function modifier_Primary_Desolate_passive:IsPurgable() 		return false end
function modifier_Primary_Desolate_passive:IsPurgeException() 	return false end

function modifier_Primary_Desolate_passive:OnDestroy()
	...
	for _ , enemy in pairs(enemies) do
		local modifier = enemy:FindModifierByName("modifier_Primary_Desolate_alone")
		if modifier then
...
function modifier_Primary_Desolate_passive:DeclareFunctions()	
	return {
		MODIFIER_PROPERTY_PROCATTACK_BONUS_DAMAGE_PHYSICAL,
	}
end

function modifier_Primary_Desolate_passive:ADDeclareFunctions()	
	return {
		MODIFIER_EVENT_ON_ATTACK_LANDED = {self:GetParent(),nil}
	}
end

function modifier_Primary_Desolate_passive:GetModifierProcAttack_BonusDamage_Physical()
	return self:GetAbility():GetSpecialValueFor("bonus_attack")*self:GetCaster():GetAverageTrueAttackDamage(nil)*0.01
end

function modifier_Primary_Desolate_passive:OnAttackLanded(keys)
	if not IsServer() then
		return 
	end
	...
	target:AddNewModifier(caster, self:GetAbility(), "modifier_Primary_Desolate_alone", {stack = damage})

end

----------------------------------------------
modifier_Primary_Desolate_alone = advanced_modifier({})

function modifier_Primary_Desolate_alone:IsDebuff()			return true end
function modifier_Primary_Desolate_alone:IsHidden() 			return false end
function modifier_Primary_Desolate_alone:IsPurgable() 		return false end
function modifier_Primary_Desolate_alone:IsPurgeException() 	return false end

function modifier_Primary_Desolate_alone:OnCreated(keys)
	if IsServer() then
...
function modifier_Primary_Desolate_alone:OnRefresh(keys)
	if IsServer() then
		self:SetStackCount(self:GetStackCount()+ keys.stack)
	end
end

function modifier_Primary_Desolate_alone:OnIntervalThink()
