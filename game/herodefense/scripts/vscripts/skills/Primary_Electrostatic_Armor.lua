Primary_Electrostatic_Armor = class({})
LinkLuaModifier("modifier_Primary_Electrostatic_Armor_damage_count", "skills/Primary_Electrostatic_Armor", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Primary_Electrostatic_Armor_spell", "skills/Primary_Electrostatic_Armor", LUA_MODIFIER_MOTION_NONE)
-- LinkLuaModifier("modifier_Primary_Electrostatic_Armor_shield", "skills/Primary_Electrostatic_Armor", LUA_MODIFIER_MOTION_NONE)

function Primary_Electrostatic_Armor:Precache( context )
...
--Abilities
function Primary_Electrostatic_Armor:IsHiddenWhenStolen() 		return false end
function Primary_Electrostatic_Armor:IsRefreshable() 			return true end
function Primary_Electrostatic_Armor:IsStealable() 				return true end
function Primary_Electrostatic_Armor:IsNetherWardStealable()		return true end
function Primary_Electrostatic_Armor:GetIntrinsicModifierName() return "modifier_Primary_Electrostatic_Armor_damage_count" end
function Primary_Electrostatic_Armor:GetAOERadius()
	return self:GetSpecialValueFor("radius")- self:GetCaster():GetCastRangeBonus()
end


modifier_Primary_Electrostatic_Armor_damage_count = class({})
function modifier_Primary_Electrostatic_Armor_damage_count:IsHidden() return false end
function modifier_Primary_Electrostatic_Armor_damage_count:IsDebuff() return false end
function modifier_Primary_Electrostatic_Armor_damage_count:IsPurgable() 		return false end
function modifier_Primary_Electrostatic_Armor_damage_count:IsPurgeException() 	return false end
function modifier_Primary_Electrostatic_Armor_damage_count:RemoveOnDeath()  return false end
function modifier_Primary_Electrostatic_Armor_damage_count:IsStunDebuff() return false end
function modifier_Primary_Electrostatic_Armor_damage_count:AllowIllusionDuplicate() return false end

function modifier_Primary_Electrostatic_Armor_damage_count:DeclareFunctions() return
    {MODIFIER_PROPERTY_MAGICAL_RESISTANCE_BONUS,MODIFIER_EVENT_ON_TAKEDAMAGE} end
function modifier_Primary_Electrostatic_Armor_damage_count:GetModifierMagicalResistanceBonus() return self:GetParent():PassivesDisabled() and 0 or self:GetAbility():GetSpecialValueFor("bonus_magic_resistance") end


function modifier_Primary_Electrostatic_Armor_damage_count:OnTakeDamage(keys)
    ...
    if self:GetStackCount() > caster:GetMaxHealth()*ability:GetSpecialValueFor("damage_require")*0.01 then
        caster:AddNewModifier(caster, ability, "modifier_Primary_Electrostatic_Armor_spell", {})
...
modifier_Primary_Electrostatic_Armor_spell = class({})
function modifier_Primary_Electrostatic_Armor_spell:IsHidden() return true end
function modifier_Primary_Electrostatic_Armor_spell:IsDebuff() return false end
function modifier_Primary_Electrostatic_Armor_spell:IsPurgable() return false end
function modifier_Primary_Electrostatic_Armor_spell:IsPurgeException() return false end
function modifier_Primary_Electrostatic_Armor_spell:IsStunDebuff() return false end
function modifier_Primary_Electrostatic_Armor_spell:AllowIllusionDuplicate() return false end

function modifier_Primary_Electrostatic_Armor_spell:OnCreated(params)
    if not IsServer() then
        return
    end
...
--可以多重
function modifier_Primary_Electrostatic_Armor_spell:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end
function modifier_Primary_Electrostatic_Armor_spell:OnIntervalThink()
	if IsServer() then
        if not self.hCaster:IsNull() and  self.hCaster:IsAlive() then
            ...
            if self.fCurDis == self.iRadius then
                -- self:GetParent():AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_Primary_Electrostatic_Armor_shield",
...
function modifier_Primary_Electrostatic_Armor_spell:OnDestroy()
	if IsServer() then
