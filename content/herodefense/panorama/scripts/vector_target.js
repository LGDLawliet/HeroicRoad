...
function StartVectorTarget( location, ability ) {
	...
	let radius = Abilities.GetSpecialValueFor( ability, "vector_radius" );
	if (radius>0) {
		Particles.SetParticleControl( effect_cast, 3, [radius, radius, radius] );
	}else{
		Particles.SetParticleControl( effect_cast, 3, [80, 80, 80] );
	}
...
function LoopVectorTarget( effect_cast, ability, location ) {
	...
	let vector_distance_max = Abilities.GetSpecialValueFor( ability, "vector_distance_max" );
	let vector_distance_min = Abilities.GetSpecialValueFor( ability, "vector_distance_min" );
	let vector_distance_all = Abilities.GetSpecialValueFor( ability, "vector_distance_all" );
	if (vector_distance_all>0) {
		vector_distance_max = vector_distance_all;
		vector_distance_min = vector_distance_all;
	}
	// set minimum position
	if (dist<100) {
		end_pos = VectorAdd( location, VectorScale( direction, 100 ) );
	}else{
		if (vector_distance_max>0 && dist>vector_distance_max) {
			end_pos = VectorAdd( location, VectorScale( direction, vector_distance_max ) );
		}
	}
	if (vector_distance_min>0 && dist<vector_distance_min) {
		end_pos = VectorAdd( location, VectorScale( direction, vector_distance_min ) );
	}
