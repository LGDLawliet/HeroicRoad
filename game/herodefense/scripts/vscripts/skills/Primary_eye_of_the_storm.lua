...
Primary_eye_of_the_storm = class({})
LinkLuaModifier( "modifier_Primary_eye_of_the_storm", "skills/Primary_eye_of_the_storm", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_Primary_eye_of_the_storm_debuff", "skills/Primary_eye_of_the_storm", LUA_MODIFIER_MOTION_NONE )

function Primary_eye_of_the_storm:Precache( context )
...
function Primary_eye_of_the_storm:OnSpellStart()
	local caster = self:GetCaster()
	local duration = self:GetSpecialValueFor( "duration" )

    local already = caster:FindModifierByName("modifier_Primary_eye_of_the_storm")
	if already then
		already:Destroy()
	end
	caster:AddNewModifier(caster, self, "modifier_Primary_eye_of_the_storm", {duration = duration})
end

modifier_Primary_eye_of_the_storm = advanced_modifier({})

function modifier_Primary_eye_of_the_storm:IsHidden() return false end
function modifier_Primary_eye_of_the_storm:IsDebuff() return false end
function modifier_Primary_eye_of_the_storm:IsPurgable() return false end
function modifier_Primary_eye_of_the_storm:OnCreated()
...
function modifier_Primary_eye_of_the_storm:OnDestroy()
...
function modifier_Primary_eye_of_the_storm:OnIntervalThink()
	local targets = {}
    ...
    if targetCount >= 1 then
        ...
        for enemy,_ in pairs(targets) do
            ...
            if enemy:IsAlive() then
                local debuff = enemy:FindModifierByName("modifier_Primary_eye_of_the_storm_debuff")
                if debuff then
                    ...
                    local newdebuff = enemy:AddNewModifier(self.parent, self, "modifier_Primary_eye_of_the_storm_debuff", {duration = self.armor_duration})
...
function modifier_Primary_eye_of_the_storm:PlayEffects1()
...
function modifier_Primary_eye_of_the_storm:PlayEffects2( enemy )
...
---
modifier_Primary_eye_of_the_storm_debuff = advanced_modifier({})

function modifier_Primary_eye_of_the_storm_debuff:IsHidden() return false end
function modifier_Primary_eye_of_the_storm_debuff:IsDebuff() return true end
function modifier_Primary_eye_of_the_storm_debuff:IsPurgable() return false end
function modifier_Primary_eye_of_the_storm_debuff:GetTexture() return "razor_eye_of_the_storm" end
function modifier_Primary_eye_of_the_storm_debuff:DeclareFunctions() return {MODIFIER_PROPERTY_TOOLTIP} end
function modifier_Primary_eye_of_the_storm_debuff:ADDeclareFunctions() return {advanced_MODIFIER_PROPERTY_PHYSICAL_ARMOR_BONUS} end
function modifier_Primary_eye_of_the_storm_debuff:Advanced_GetModifierPhysicalArmorBonus() return -self:GetStackCount() end
function modifier_Primary_eye_of_the_storm_debuff:OnTooltip() return self:GetStackCount() end
